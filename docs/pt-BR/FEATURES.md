# Referência de Recursos do BRIEF

Visão em Português dos recursos centrais do BRIEF.  
...
- **UAT guiado** via `/brief-verify-work`
- **Suporte brownfield** com `/brief-map-codebase`
...
- **`/brief-ui-phase`**: contrato visual antes da execução
- **`/brief-ui-review`**: auditoria visual em 6 pilares
...
- **Diagnóstico forense** com `/brief-forensics`
- **Relatório de sessão** com `/brief-session-report`

## Novidades v1.31--v1.32

- **Schema drift detection** — detecta alterações em ORM schema sem migração correspondente
- **Security enforcement** — verificação de segurança ancorada em threat model (`/brief-secure-phase`)
...
- **`--diagnose` flag** — modo diagnóstico sem modificações no `/brief-debug` (v1.32)
- **`/brief-analyze-dependencies`** — detecta dependências entre fases (v1.32)
...
| Cenário | Comandos |
|--------|----------|
| Projeto novo | `/brief-new-project` -> `/brief-discuss-phase` -> `/brief-plan-phase` -> `/brief-execute-phase` |
| Correção rápida | `/brief-quick` |
| Código existente | `/brief-map-codebase` -> `/brief-new-project` |
| Fechamento de release | `/brief-audit-milestone` -> `/brief-complete-milestone` |
