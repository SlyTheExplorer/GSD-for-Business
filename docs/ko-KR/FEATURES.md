...
**명령어:** `/brief-new-project [--auto @file.md]`
...
- 기존 코드베이스 컨텍스트(`/brief-map-codebase`에서)가 있으면 로드됩니다.

---

### 2. Phase Discussion

**명령어:** `/brief-discuss-phase [N] [--auto] [--batch]`
...
**명령어:** `/brief-plan-phase [N] [--auto] [--skip-research] [--skip-verify]`
...
- REQ-PLAN-07: 프론트엔드 페이즈가 감지되고 UI-SPEC.md가 없는 경우 `/brief-ui-phase` 실행을 촉구해야 합니다(UI 안전 게이트).
...
**명령어:** `/brief-audit-milestone`, `/brief-complete-milestone`, `/brief-new-milestone [name]`
...
**갭 해소.** `/brief-plan-milestone-gaps`는 감사에서 식별된 갭을 해소하는 페이즈를 생성합니다.
...
**명령어:** `/brief-add-phase`, `/brief-insert-phase [N]`, `/brief-remove-phase [N]`
...
**명령어:** `/brief-quick [--full] [--discuss] [--research]`

**목적:** BRIEF 보증을 제공하지만 더 빠른 경로로 임시 작업을 실행합니다.
...
**목적:** 자유형 텍스트를 분석하고 적절한 BRIEF 명령어로 라우팅합니다.

**요구사항.**
- REQ-DO-01: 자연어 입력에서 사용자 의도를 파악해야 합니다.
- REQ-DO-02: 의도를 가장 적합한 BRIEF 명령어에 매핑해야 합니다.
...
- REQ-NEXT-04: 프로젝트가 없으면 `/brief-new-project`를 제안해야 합니다.
- REQ-NEXT-05: 모든 페이즈가 완료되면 `/brief-complete-milestone`을 제안해야 합니다.

**상태 감지 로직.**
| 상태 | 액션 |
|-------|--------|
| `.planning/` 디렉토리 없음 | `/brief-new-project` 제안 |
| 페이즈에 CONTEXT.md 없음 | `/brief-discuss-phase` 실행 |
| 페이즈에 PLAN.md 파일 없음 | `/brief-plan-phase` 실행 |
| 계획 있지만 SUMMARY.md 없음 | `/brief-execute-phase` 실행 |
| 실행되었지만 VERIFICATION.md 없음 | `/brief-verify-work` 실행 |
| 모든 페이즈 완료 | `/brief-complete-milestone` 제안 |
...
- REQ-POSTVER-03: `/brief-verify-work`가 처리할 문제를 기록해야 합니다.
...
**실행 시점.** `/brief-execute-phase` 중 검증자 단계 전에 자동으로 실행됩니다.
...
**실행 시점.** `/brief-plan-phase`의 계획 검사기 루프 후 자동으로 실행됩니다.
...
- REQ-CTX-06: 컨텍스트 모니터는 BRIEF 활성 프로젝트와 비활성 프로젝트를 구분해야 합니다.
...
**명령어:** `/brief-pause-work`, `/brief-resume-work`, `/brief-progress`
...
**명령어:** `/brief-set-profile <quality|balanced|budget|inherit>`
...
**명령어:** `/brief-map-codebase [area]`

**목적:** 새 프로젝트를 시작하기 전에 기존 코드베이스를 분석하여 BRIEF가 무엇이 존재하는지 이해하도록 합니다.
...
- REQ-MAP-04: 이후 `/brief-new-project`는 코드베이스 매핑을 로드하고 추가하는 내용에 대한 질문에 집중해야 합니다.
...
**명령어:** `/brief-add-todo [desc]`, `/brief-check-todos`
...
**목적:** 변경 로그 미리보기와 함께 BRIEF를 최신 버전으로 업데이트합니다.
...
- REQ-UPDATE-05: `/brief-reapply-patches`는 업데이트 후 로컬 수정사항을 복원해야 합니다.
...
**목적:** 여러 AI 코딩 에이전트 런타임에서 BRIEF를 실행합니다.
...
- REQ-RUNTIME-05: 제거는 다른 구성에 영향을 주지 않고 모든 BRIEF 파일을 깔끔하게 제거해야 합니다.
...
**명령어:** `/brief-profile-user [--questionnaire] [--refresh]`
...
- `/brief-dev-preferences` 명령어 — 모든 세션에서 선호도 로드
...
- `brief-user-profiler` 에이전트 — 세션 데이터에서 행동 분석
...
모든 `/brief-progress` 호출은 현재 마일스톤의 모든 페이즈에서 미결 항목(pending, skipped, blocked, human_needed)을 스캔합니다. 실행 가능한 링크가 포함된 비차단 경고 섹션을 표시합니다.
...
**요구사항.**
- REQ-DEBT-01: `/brief-progress`에서 모든 이전 페이즈의 미결 UAT/검증 항목을 표시해야 합니다.
...
- REQ-DEBT-06: `/brief-audit-uat`는 모든 페이즈를 스캔하고 테스트 가능성별로 항목을 분류하며 사람 테스트 계획을 생성해야 합니다.
...
**명령어:** `/brief-fast [task description]`
...
**`/brief-quick`과 비교하여 사용 시점.**
- `/brief-fast` — 2분 이내에 실행 가능한 한 문장 작업(오타, 구성 변경, 작은 추가)
- `/brief-quick` — 연구, 다단계 계획, 또는 검증이 필요한 모든 것

---

### 42. Cross-AI Peer Review

**명령어:** `/brief-review --phase N [--gemini] [--claude] [--codex] [--coderabbit] [--opencode] [--qwen] [--cursor] [--all]`
...
- REQ-REVIEW-05: 검토는 `/brief-plan-phase --reviews`가 사용할 수 있어야 합니다.
...
**명령어:** `/brief-add-backlog <description>`, `/brief-review-backlog`, `/brief-plant-seed <idea>`
...
- REQ-BACKLOG-02: `/brief-discuss-phase`와 `/brief-plan-phase`가 작동할 수 있도록 페이즈 디렉토리를 즉시 생성해야 합니다.
- REQ-BACKLOG-03: `/brief-review-backlog`는 항목별로 승격, 유지, 제거 액션을 지원해야 합니다.
- REQ-BACKLOG-04: 승격된 항목은 활성 마일스톤 순서로 번호가 다시 매겨져야 합니다.
- REQ-SEED-01: 시드는 표시 조건에 대한 전체 이유와 시기를 캡처해야 합니다.
- REQ-SEED-02: `/brief-new-milestone`은 시드를 스캔하고 일치하는 항목을 표시해야 합니다.
...
**명령어:** `/brief-thread [name | description]`

**목적:** 여러 세션에 걸쳐 있지만 특정 페이즈에 속하지 않는 작업을 위한 가벼운 교차 세션 지식 저장소입니다. `/brief-pause-work`보다 더 가볍습니다 — 페이즈 상태나 계획 컨텍스트가 없습니다.
...
**명령어:** `/brief-pr-branch [target branch]`

**목적:** `.planning/` 커밋을 필터링하여 풀 리퀘스트에 적합한 깔끔한 브랜치를 생성합니다. 검토자는 BRIEF 계획 산출물이 아닌 코드 변경사항만 봅니다.
...
**목적:** BRIEF 계획 산출물에 대한 심층 방어 보안. BRIEF가 LLM 시스템 프롬프트가 되는 마크다운 파일을 생성하기 때문에, 이 파일로 흘러드는 사용자 제어 텍스트는 잠재적인 간접 프롬프트 주입 벡터입니다.
...
Claude가 BRIEF 워크플로우 컨텍스트 밖에서 파일 편집을 시도하는 것을 감지하는 PreToolUse 훅입니다. 직접 편집 대신 `/brief-quick` 또는 `/brief-fast` 사용을 권고합니다. `hooks.workflow_guard`로 구성 가능합니다(기본값: false).
...
**목적:** `/brief-discuss-phase` 중에 `DISCUSSION-LOG.md`를 자동 생성하여 논의 중에 내려진 결정의 전체 감사 추적을 제공합니다.
...
**명령어:** `/brief-forensics [description]`

**목적:** 실패하거나 막힌 BRIEF 워크플로우의 사후 조사.
...
**명령어:** `/brief-milestone-summary [version]`
...
2. **전환** — 이후 BRIEF 명령어를 위한 활성 워크스트림 컨텍스트 변경
...
**명령어:** `/brief-discuss-phase` with `workflow.discuss_mode: 'assumptions'`
...
**일부:** `/brief-new-project` 및 `/brief-progress`

**목적:** UI 중심 프로젝트를 자동으로 감지하고 `/brief-ui-phase` 권장사항을 표시합니다.

**요구사항.**
- REQ-UI-DETECT-01: 프로젝트 설명에서 UI 신호를 감지해야 합니다(키워드, 프레임워크 참조).
- REQ-UI-DETECT-02: 해당하는 경우 ROADMAP.md 페이즈에 `ui_hint`를 주석으로 추가해야 합니다.
- REQ-UI-DETECT-03: UI 중심 페이즈의 다음 단계에서 `/brief-ui-phase`를 제안해야 합니다.
- REQ-UI-DETECT-04: `/brief-ui-phase`를 필수로 만들어서는 안 됩니다.

**프로세스.**
1. **감지** — UI 신호(키워드, 프레임워크 참조)에 대한 프로젝트 설명 및 기술 스택 스캔
2. **주석** — ROADMAP.md의 해당 페이즈에 `ui_hint` 표시 추가
3. **표시** — UI 중심 페이즈의 다음 단계에 `/brief-ui-phase` 권장사항 포함
...
3. **설치** — 단일 세션에서 선택된 모든 런타임에 BRIEF 구성
...
2. **변환** — BRIEF 프롬프트를 Windsurf 규칙 형식으로 변환
3. **설치** — Windsurf 구성 디렉토리에 BRIEF 설정
...
**목적:** BRIEF 문서를 포르투갈어, 한국어, 일본어로 제공합니다.
...
**목적:** CLI 세션 없이 프로그래밍 방식으로 BRIEF 워크플로우를 실행하기 위한 헤드리스 TypeScript SDK.

**요구사항.**
- REQ-SDK-01: SDK는 BRIEF 워크플로우 작업을 TypeScript 함수로 노출해야 합니다.
- REQ-SDK-02: SDK는 대화형 프롬프트 없이 헤드리스 실행을 지원해야 합니다.
- REQ-SDK-03: SDK는 CLI 기반 워크플로우와 동일한 아티팩트를 생성해야 합니다.

**프로세스.**
1. **임포트** — TypeScript/JavaScript 프로젝트에 BRIEF SDK 임포트
2. **구성** — 프로젝트 경로와 워크플로우 옵션을 프로그래밍 방식으로 설정
3. **실행** — API 호출로 BRIEF 페이즈(discuss, plan, execute) 실행
...
**명령어:** `/brief-execute-phase` 실행 시 자동
...
**명령어:** `/brief-docs-update`

**목적:** 정확성 검사가 포함된 프로젝트 문서를 생성하고 검증합니다.

**요구사항.**
- REQ-DOCS-01: 시스템은 문서 생성을 위해 `brief-doc-writer` 에이전트를 스폰해야 합니다.
- REQ-DOCS-02: 시스템은 정확성 검사를 위해 `brief-doc-verifier` 에이전트를 스폰해야 합니다.
...
**프로세스.**
1. **생성** — `brief-doc-writer`를 스폰하여 구현에서 문서 생성 또는 업데이트
2. **검증** — `brief-doc-verifier`를 스폰하여 코드베이스에 대한 문서 정확성 검사
...
**플래그:** `/brief-discuss-phase <N> --chain`
...
**목적:** BRIEF 명령어를 하위 호환성을 유지하면서 Claude Code 2.1.88+ 스킬 형식으로 마이그레이션합니다.

**요구사항.**
- REQ-SKILLS-01: 설치 프로그램은 Claude Code 2.1.88+ 용 `skills/brief-*/SKILL.md`를 작성해야 합니다.
...
2. **마이그레이션** — 각 BRIEF 명령어에 대해 `skills/brief-*/SKILL.md` 파일 작성
...
**명령어:** `/brief-analyze-dependencies`

**목적:** 페이즈 의존성을 감지하고 `/brief-manager` 실행 전 ROADMAP.md에 `Depends on` 항목을 제안합니다.
...
**목적:** Trae IDE, Cline, Augment Code 런타임으로 BRIEF 설치를 확장합니다.
