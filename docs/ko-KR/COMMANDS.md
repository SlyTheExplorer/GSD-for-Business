...
```bash
/brief-new-project                    # 대화형 모드
/brief-new-project --auto @prd.md     # PRD에서 자동 추출
```
...
/brief-new-workspace --name feature-b --repos . --strategy worktree  # 동일 저장소 격리
/brief-new-workspace --name spike --repos api,web --strategy clone   # 전체 클론
```
...
활성 BRIEF 워크스페이스와 상태를 목록으로 표시합니다.

**스캔 위치:** `~/brief-workspaces/`에서 `WORKSPACE.md` 매니페스트를 탐색합니다.
**표시 항목:** 이름, 저장소 수, 전략, BRIEF 프로젝트 상태
...
```bash
/brief-discuss-phase 1                # 페이즈 1 대화형 토론
/brief-discuss-phase 3 --auto         # 페이즈 3 기본값 자동 선택
/brief-discuss-phase --batch          # 현재 페이즈 일괄 모드
/brief-discuss-phase 2 --analyze      # 트레이드오프 분석 포함 토론
```
...
```bash
/brief-ui-phase 2                     # 페이즈 2 설계 계약 생성
```
...
```bash
/brief-plan-phase 1                   # 페이즈 1 조사 + 계획 + 검증
/brief-plan-phase 3 --skip-research   # 조사 없이 계획 (익숙한 도메인)
/brief-plan-phase --auto              # 비대화형 계획 수립
```
...
```bash
/brief-execute-phase 1                # 페이즈 1 실행
/brief-execute-phase 1 --wave 2       # Wave 2만 실행
```
...
**동작 방식.**
- 프로젝트 없음 → `/brief-new-project` 제안
- 페이즈 토론 필요 → `/brief-discuss-phase` 실행
- 페이즈 계획 필요 → `/brief-plan-phase` 실행
- 페이즈 실행 필요 → `/brief-execute-phase` 실행
- 페이즈 검증 필요 → `/brief-verify-work` 실행
...
```bash
/brief-session-report                 # 세션 종료 후 요약 생성
```
...
**사전 조건:** 페이즈 검증 완료 (`/brief-verify-work` 통과), `gh` CLI 설치 및 인증
**생성 파일:** 계획 아티팩트 기반의 풍부한 본문이 포함된 GitHub PR, STATE.md 업데이트

```bash
/brief-ship 4                         # 페이즈 4 출시
/brief-ship 4 --draft                 # 초안 PR로 출시
```
...
**사전 조건:** 프론트엔드 코드가 있는 프로젝트 (독립 실행 가능, BRIEF 프로젝트 불필요)
**생성 파일:** `{phase}-UI-REVIEW.md`, `.planning/ui-reviews/`에 스크린샷

```bash
/brief-ui-review                      # 현재 페이즈 감사
/brief-ui-review 3                    # 페이즈 3 감사
```
...
```bash
/brief-milestone-summary                # 현재 마일스톤 요약
/brief-milestone-summary v1.0           # 특정 마일스톤 요약
```
...
```bash
/brief-new-milestone                  # 대화형 모드
/brief-new-milestone "v2.0 Mobile"    # 이름이 지정된 마일스톤
/brief-new-milestone --reset-phase-numbers "v2.0 Mobile"  # 마일스톤 번호를 1부터 재시작
```
...
```bash
/brief-add-phase                      # 대화형 — 페이즈를 설명합니다
```
...
```bash
/brief-insert-phase 3                 # 페이즈 3과 4 사이에 삽입 → 3.1 생성
```
...
```bash
/brief-remove-phase 7                 # 페이즈 7 제거, 8→7, 9→8 등으로 재번호
```
...
```bash
/brief-list-phase-assumptions 2       # 페이즈 2 가정 사항 확인
```
...
```bash
/brief-plan-milestone-gaps             # 각 감사 갭에 대한 페이즈 생성
```

### `/brief-research-phase`

심층 에코시스템 조사만 수행합니다 (독립 실행 — 일반적으로 `/brief-plan-phase`를 사용하세요).
...
```bash
/brief-research-phase 4               # 페이즈 4 도메인 조사
```
...
```bash
/brief-validate-phase 2               # 페이즈 2 테스트 커버리지 감사
```
...
```bash
/brief-progress                       # "지금 어디 있나? 다음은 무엇인가?"
```
...
```bash
/brief-resume-work                    # 컨텍스트 초기화 또는 새 세션 후 실행
```
...
```bash
/brief-pause-work                     # continue-here.md 생성
```
...
```bash
/brief-analyze-dependencies            # 의존성 분석 및 제안
```
...
```bash
/brief-quick                          # 기본 빠른 작업
/brief-quick --discuss --research     # 토론 + 조사 + 계획
/brief-quick --full                   # 계획 검사 및 검증 포함
/brief-quick --discuss --research --full  # 모든 선택적 단계 포함
```
...
```bash
/brief-autonomous                     # 남은 모든 페이즈 실행
/brief-autonomous --from 3            # 페이즈 3부터 시작
/brief-autonomous --to 5              # 페이즈 5까지만 실행
/brief-autonomous --from 3 --to 5     # 페이즈 3~5 범위 실행
/brief-autonomous --only 4            # 페이즈 4만 자율 실행
```

### `/brief-do`

자유 형식 텍스트를 적절한 BRIEF 명령어로 라우팅합니다.

```bash
/brief-do                             # 원하는 작업을 설명합니다
```
...
```bash
/brief-add-tests 2                    # 페이즈 2 테스트 생성
```
...
- `/brief-dev-preferences` 명령어 — 모든 세션에서 선호도를 로드합니다
- `CLAUDE.md` 프로필 섹션 — Claude Code에 의해 자동으로 인식됩니다

```bash
/brief-profile-user                   # 세션 분석 및 프로필 구축
/brief-profile-user --questionnaire   # 대화형 설문지 대체 방법
/brief-profile-user --refresh         # 새로운 분석으로 재생성
```
...
실패하거나 멈춘 BRIEF 워크플로우에 대한 사후 조사를 수행합니다.
...
```bash
/brief-forensics                              # 대화형 — 문제 입력 프롬프트
/brief-forensics "Phase 3 execution stalled"  # 문제 설명과 함께 실행
```
...
```bash
/brief-workstreams                    # 모든 워크스트림 목록
/brief-workstreams create backend-api # 새 워크스트림 생성
/brief-workstreams switch backend-api # 활성 워크스트림 설정
/brief-workstreams status backend-api # 상세 상태 확인
/brief-workstreams progress           # 교차 워크스트림 진행 상황 개요
/brief-workstreams complete backend-api  # 완료된 워크스트림 아카이브
/brief-workstreams resume backend-api    # 워크스트림 작업 재개
```
...
```bash
/brief-set-profile budget             # 예산 프로필로 전환
/brief-set-profile quality            # 품질 프로필로 전환
```
...
```bash
/brief-map-codebase                   # 전체 코드베이스 분석
/brief-map-codebase auth              # 인증 영역에 집중
```
...
변경 로그 미리보기와 함께 BRIEF를 업데이트합니다.

```bash
/brief-update                         # 업데이트 확인 및 설치
```

### `/brief-reapply-patches`

BRIEF 업데이트 후 로컬 수정사항을 복원합니다.

```bash
/brief-reapply-patches                # 로컬 변경사항 병합
```
...
**`/brief-quick`의 대체가 아닙니다.** 조사, 다단계 계획 또는 검증이 필요한 작업에는 `/brief-quick`을 사용하세요.
...
**생성 파일:** `{phase}-REVIEWS.md` — `/brief-plan-phase --reviews`에서 사용 가능
...
**목적:** 리뷰어에게 BRIEF 계획 아티팩트가 아닌 코드 변경사항만 표시합니다.

```bash
/brief-pr-branch                     # main을 기준으로 필터링
/brief-pr-branch develop             # develop을 기준으로 필터링
```
...
**999.x 번호 체계**는 백로그 항목을 활성 페이즈 순서 밖에 유지합니다. 페이즈 디렉터리가 즉시 생성되므로 해당 항목에 대해 `/brief-discuss-phase`와 `/brief-plan-phase`를 사용할 수 있습니다.
...
**사용처:** `/brief-new-milestone` (시드를 스캔하여 일치 항목 제시)
...
스레드는 여러 세션에 걸쳐 이어지지만 특정 페이즈에 속하지 않는 작업을 위한 경량 교차 세션 지식 저장소입니다. `/brief-pause-work`보다 가볍습니다.

```bash
/brief-thread                         # 모든 스레드 목록
/brief-thread fix-deploy-key-auth     # 스레드 재개
/brief-thread "Investigate TCP timeout in pasta service"  # 새 스레드 생성
```
