...
BRIEF는 사용자와 AI 코딩 에이전트(Claude Code, Gemini CLI, OpenCode, Kilo, Codex, Copilot, Antigravity, Trae, Cline, Augment Code) 사이에 위치하는 **메타 프롬프팅 프레임워크**입니다. 다음을 제공합니다.
...
워크플로우 파일(`brief/workflows/*.md`)은 무거운 작업을 직접 수행하지 않습니다. 다음 작업만 담당합니다.
- `brief-tools.cjs init <workflow>`로 컨텍스트를 로드합니다
...
- **OpenCode / Kilo:** 슬래시 명령어 (`/brief-command-name`)
- **Codex:** Skills (`$gsd-command-name`)
- **Copilot:** 슬래시 명령어 (`/brief-command-name`)
- **Antigravity:** Skills
...
- `brief-tools.cjs init`을 통한 컨텍스트 로드
...
모든 계획 아티팩트를 위한 Markdown 템플릿입니다. `brief-tools.cjs template fill`과 `scaffold` 명령어가 사전 구조화된 파일을 생성하는 데 사용합니다.
...
| `gsd-check-update.js` | `SessionStart` | 새 BRIEF 버전을 백그라운드에서 확인 |
| `gsd-prompt-guard.js` | `PreToolUse` | `.planning/` 쓰기 작업에서 프롬프트 인젝션 패턴 스캔 (권고용) |
| `gsd-workflow-guard.js` | `PreToolUse` | BRIEF 워크플로우 컨텍스트 외부의 파일 편집 감지 (권고용, `hooks.workflow_guard`로 활성화) |

### CLI Tools (`brief/bin/`)

17개의 도메인 모듈을 포함하는 Node.js CLI 유틸리티(`brief-tools.cjs`)입니다.
...
| 카테고리 | 에이전트 | 병렬성 |
|----------|--------|-------------|
| **Synthesizers** | brief-research-synthesizer | 순차적 (조사자 완료 후) |
| **Planners** | brief-planner, brief-roadmapper | 순차적 |
| **Executors** | brief-executor | 웨이브 내 병렬, 웨이브 간 순차적 |
| **Verifiers** | brief-verifier | 순차적 (모든 executor 완료 후) |
| **Mappers** | brief-codebase-mapper | 4개 병렬 (tech, arch, quality, concerns) |
...
├── research/               # /brief-new-project의 도메인 조사
│   ├── SUMMARY.md
│   ├── STACK.md
│   ├── FEATURES.md
│   ├── ARCHITECTURE.md
│   └── PITFALLS.md
├── codebase/               # 브라운필드 매핑 (/brief-map-codebase에서)
│   ├── STACK.md
│   ├── ARCHITECTURE.md
│   ├── CONVENTIONS.md
│   ├── CONCERNS.md
...
│   ├── pending/            # 캡처된 아이디어
│   └── done/               # 완료된 할 일
├── threads/               # 영구 컨텍스트 스레드 (/brief-thread에서)
├── seeds/                 # 미래 지향적 아이디어 (/brief-plant-seed에서)
├── debug/                  # 활성 디버그 세션
│   ├── *.md                # 활성 세션
│   ├── resolved/           # 보관된 세션
│   └── knowledge-base.md   # 영구 디버그 학습 내용
├── ui-reviews/             # /brief-ui-review의 스크린샷 (gitignored)
...
7. **패치 백업** — v1.17부터 로컬 수정 파일을 `gsd-local-patches/`에 백업하여 `/brief-reapply-patches`에 사용
8. **매니페스트 추적** — 깔끔한 제거를 위해 `gsd-file-manifest.json` 작성
9. **제거 모드** — `--uninstall`로 모든 BRIEF 파일, 훅, 설정 제거
...
- BRIEF 워크플로우 컨텍스트 외부의 편집을 감지합니다 (활성 `/brief-` 명령어 또는 Task 서브에이전트 없음)
- 상태 추적 변경을 위해 `/brief-quick` 또는 `/brief-fast` 사용을 권고합니다
...
BRIEF는 통합된 명령어/워크플로우 아키텍처를 통해 여러 AI 코딩 런타임을 지원합니다.

| 런타임 | 명령어 형식 | 에이전트 시스템 | 설정 위치 |
|---------|---------------|--------------|-----------------|
| Claude Code | `/brief-command` | Task 생성 | `~/.claude/` |
| OpenCode | `/brief-command` | Subagent 모드 | `~/.config/opencode/` |
| Kilo | `/brief-command` | Subagent 모드 | `~/.config/kilo/` |
| Gemini CLI | `/brief-command` | Task 생성 | `~/.gemini/` |
| Codex | `$gsd-command` | Skills | `~/.codex/` |
| Copilot | `/brief-command` | 에이전트 위임 | `~/.github/` |
...
5. **모델 참조** — `inherit` 프로필을 통해 BRIEF가 런타임의 모델 선택에 위임합니다
