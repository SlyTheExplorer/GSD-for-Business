...
BRIEF는 프로젝트 설정을 `.planning/config.json`에 저장합니다. `/brief-new-project` 실행 시 생성되며 `/brief-settings`를 통해 업데이트할 수 있습니다.
...
| `workflow.ui_safety_gate` | boolean | `true` | plan 단계에서 프론트엔드 단계에 대해 /brief-ui-phase 실행 여부 확인 |
...
| `workflow.discuss_mode` | string | `'discuss'` | `/brief-discuss-phase`의 컨텍스트 수집 방식을 제어합니다. `'discuss'` (기본값)는 질문을 하나씩 합니다. `'assumptions'`는 코드베이스를 먼저 읽고 신뢰도 수준이 있는 구조화된 가정을 생성하여 틀린 부분만 수정하도록 요청합니다. v1.28에서 추가 |
| `workflow.skip_discuss` | boolean | `false` | `true`로 설정하면 `/brief-autonomous`가 discuss 단계를 완전히 건너뛰고 ROADMAP 단계 목표로부터 최소한의 CONTEXT.md를 작성합니다. 개발자 선호사항이 PROJECT.md/REQUIREMENTS.md에 모두 캡처된 프로젝트에 유용합니다. v1.28에서 추가 |
...
| `hooks.workflow_guard` | boolean | `false` | BRIEF 워크플로우 컨텍스트 밖에서 파일 편집이 발생할 때 경고 ((`/brief-quick` 또는 `/brief-fast` 사용 권고)) |
...
| `git.quick_branch_template` | string 또는 null | `null` | `/brief-quick` 태스크를 위한 선택적 브랜치 이름 템플릿 |
...
비 Claude 런타임에 BRIEF를 설치하면 인스톨러가 자동으로 `~/.gsd/defaults.json`에 `resolve_model_ids: "omit"`을 설정합니다. 이로 인해 BRIEF는 모든 에이전트에 빈 model 파라미터를 반환하며 각 에이전트는 런타임에 설정된 모델을 사용합니다. 기본 사용 시 추가 설정은 필요하지 않습니다.
...
`/brief-new-project`가 새 `config.json`을 생성할 때 전역 기본값을 읽어 초기 설정으로 병합합니다. 프로젝트별 설정은 항상 전역 설정보다 우선합니다.
