...
BRIEFは、ユーザーとAIコーディングエージェント（Claude Code、Gemini CLI、OpenCode、Kilo、Codex、Copilot、Antigravity、Trae、Cline、Augment Code）の間に位置する**メタプロンプティングフレームワーク**です。以下の機能を提供します：
...
ワークフローファイル（`brief/workflows/*.md`）は重い処理を行いません。以下の役割に徹します：
- `brief-tools.cjs init <workflow>` でコンテキストを読み込む
...
- **Claude Code:** カスタムスラッシュコマンド（`/brief-command-name`）
- **OpenCode / Kilo:** スラッシュコマンド（`/brief-command-name`）
- **Codex:** スキル（`$gsd-command-name`）
- **Copilot:** スラッシュコマンド（`/brief-command-name`）
- **Antigravity:** スキル

**コマンド総数:** 44

### ワークフロー（`brief/workflows/*.md`）

コマンドが参照するオーケストレーションロジックです。以下を含むステップバイステップのプロセスが記述されています：
- `brief-tools.cjs init` によるコンテキスト読み込み
- モデル解決を伴うエージェント起動の指示
- ゲート/チェックポイントの定義
- 状態更新パターン
- エラーハンドリングとリカバリー
...
### テンプレート（`brief/templates/`）

すべてのプランニングアーティファクト用のMarkdownテンプレートです。`brief-tools.cjs template fill` および `scaffold` コマンドにより、事前構造化されたファイルを作成するために使用されます：
...
| `gsd-check-update.js` | `SessionStart` | BRIEFの新バージョンをバックグラウンドで確認 |
| `gsd-prompt-guard.js` | `PreToolUse` | `.planning/` への書き込みにプロンプトインジェクションパターンがないかスキャン（アドバイザリー） |
| `gsd-workflow-guard.js` | `PreToolUse` | BRIEFワークフローコンテキスト外でのファイル編集を検出（アドバイザリー、`hooks.workflow_guard` によるオプトイン） |

### CLIツール（`brief/bin/`）

17のドメインモジュールを持つNode.js CLIユーティリティ（`brief-tools.cjs`）：
...
| カテゴリ | エージェント | 並列実行 |
|----------|--------|-------------|
| **シンセサイザー** | brief-research-synthesizer | 逐次（リサーチャー完了後） |
| **プランナー** | brief-planner, brief-roadmapper | 逐次 |
| **エグゼキューター** | brief-executor | ウェーブ内は並列、ウェーブ間は逐次 |
| **ベリファイアー** | brief-verifier | 逐次（全エグゼキューター完了後） |
| **マッパー** | brief-codebase-mapper | 4並列（tech、arch、quality、concerns） |
...
├── research/               # /brief-new-project によるドメインリサーチ
│   ├── SUMMARY.md
│   ├── STACK.md
│   ├── FEATURES.md
│   ├── ARCHITECTURE.md
│   └── PITFALLS.md
├── codebase/               # ブラウンフィールドマッピング（/brief-map-codebase から）
│   ├── STACK.md
│   ├── ARCHITECTURE.md
│   ├── CONVENTIONS.md
│   ├── CONCERNS.md
...
│   └── done/               # 完了済みtodo
├── threads/               # 永続コンテキストスレッド（/brief-thread から）
├── seeds/                 # 将来に向けたアイデア（/brief-plant-seed から）
├── debug/                  # アクティブなデバッグセッション
│   ├── *.md                # アクティブセッション
│   ├── resolved/           # アーカイブ済みセッション
│   └── knowledge-base.md   # 永続的なデバッグ知見
├── ui-reviews/             # /brief-ui-review からのスクリーンショット（gitignore対象）
...
7. **パッチバックアップ** — v1.17以降、ローカルで変更されたファイルを `/brief-reapply-patches` 用に `gsd-local-patches/` へバックアップ
8. **マニフェスト追跡** — クリーンアンインストールのために `gsd-file-manifest.json` を書き込み
9. **アンインストールモード** — `--uninstall` ですべてのBRIEFファイル、フック、設定を削除
...
- BRIEFワークフローコンテキスト外での編集を検出（アクティブな `/brief-` コマンドやTaskサブエージェントがない場合）
- 状態追跡される変更には `/brief-quick` や `/brief-fast` の使用をアドバイス
...
BRIEFは統一されたコマンド/ワークフローアーキテクチャを通じて複数のAIコーディングランタイムをサポートしています：

| ランタイム | コマンド形式 | エージェントシステム | 設定場所 |
|---------|---------------|--------------|-----------------|
| Claude Code | `/brief-command` | Task起動 | `~/.claude/` |
| OpenCode | `/brief-command` | サブエージェントモード | `~/.config/opencode/` |
| Kilo | `/brief-command` | サブエージェントモード | `~/.config/kilo/` |
| Gemini CLI | `/brief-command` | Task起動 | `~/.gemini/` |
| Codex | `$gsd-command` | スキル | `~/.codex/` |
| Copilot | `/brief-command` | エージェント委譲 | `~/.github/` |
...
5. **モデル参照** — `inherit` プロファイルにより、BRIEFはランタイムのモデル選択に委譲
