...
| CRITICAL | <= 25% | 即座に停止し、状態を保存する（`/brief-pause-work`） |
...
BRIEF の `/brief-pause-work` コマンドは実行状態を保存します。WARNING メッセージはこのコマンドの使用を提案し、CRITICAL メッセージは即座の状態保存を指示します。

## セットアップ

両フックは `npx brief-cc` のインストール時に自動的に登録されます:
