...
```bash
/brief-new-project                    # 対話モード
/brief-new-project --auto @prd.md     # PRDから自動抽出
```
...
/brief-new-workspace --name feature-b --repos . --strategy worktree  # 同一リポジトリの分離
/brief-new-workspace --name spike --repos api,web --strategy clone   # フルクローン
```
...
アクティブなBRIEFワークスペースとそのステータスを一覧表示します。

**スキャン対象:** `~/brief-workspaces/` 内の `WORKSPACE.md` マニフェスト
**表示内容:** 名前、リポジトリ数、戦略、BRIEFプロジェクトのステータス
...
```bash
/brief-discuss-phase 1                # フェーズ1の対話的ディスカッション
/brief-discuss-phase 3 --auto         # フェーズ3でデフォルトを自動選択
/brief-discuss-phase --batch          # 現在のフェーズのバッチモード
/brief-discuss-phase 2 --analyze      # トレードオフ分析付きディスカッション
```
...
```bash
/brief-ui-phase 2                     # フェーズ2のデザイン契約書
```
...
```bash
/brief-plan-phase 1                   # フェーズ1の調査＋計画＋検証
/brief-plan-phase 3 --skip-research   # 調査なしで計画（馴染みのあるドメイン）
/brief-plan-phase --auto              # 非対話型の計画
```
...
```bash
/brief-execute-phase 1                # フェーズ1を実行
/brief-execute-phase 1 --wave 2       # ウェーブ2のみを実行
```
...
**動作:**
- プロジェクトなし → `/brief-new-project` を提案
- フェーズにディスカッションが必要 → `/brief-discuss-phase` を実行
- フェーズに計画が必要 → `/brief-plan-phase` を実行
- フェーズに実行が必要 → `/brief-execute-phase` を実行
- フェーズに検証が必要 → `/brief-verify-work` を実行
- 全フェーズ完了 → `/brief-complete-milestone` を提案
...
```bash
/brief-session-report                 # セッション後のサマリーを生成
```
...
**前提条件:** フェーズが検証済み（`/brief-verify-work` が合格）、`gh` CLIがインストールされ認証済みであること
**生成物:** 計画アーティファクトからリッチな本文を持つGitHub PR、STATE.mdの更新

```bash
/brief-ship 4                         # フェーズ4をシップ
/brief-ship 4 --draft                 # ドラフトPRとしてシップ
```
...
**前提条件:** プロジェクトにフロントエンドコードがあること（単体で動作、BRIEFプロジェクト不要）
**生成物:** `{phase}-UI-REVIEW.md`、`.planning/ui-reviews/` 内のスクリーンショット

```bash
/brief-ui-review                      # 現在のフェーズを監査
/brief-ui-review 3                    # フェーズ3を監査
```
...
```bash
/brief-milestone-summary                # 現在のマイルストーンをサマリー
/brief-milestone-summary v1.0           # 特定のマイルストーンをサマリー
```
...
```bash
/brief-new-milestone                  # 対話モード
/brief-new-milestone "v2.0 Mobile"    # 名前付きマイルストーン
/brief-new-milestone --reset-phase-numbers "v2.0 Mobile"  # マイルストーン番号を1からリスタート
```
...
```bash
/brief-add-phase                      # 対話型 — フェーズの説明を入力
```
...
```bash
/brief-insert-phase 3                 # フェーズ3と4の間に挿入 → 3.1を作成
```
...
```bash
/brief-remove-phase 7                 # フェーズ7を削除、8→7、9→8等に番号振り直し
```
...
```bash
/brief-list-phase-assumptions 2       # フェーズ2の前提を確認
```
...
```bash
/brief-plan-milestone-gaps             # 各監査ギャップに対してフェーズを作成
```

### `/brief-research-phase`

詳細なエコシステム調査のみを実行します（単体機能 — 通常は `/brief-plan-phase` を使用してください）。
...
```bash
/brief-research-phase 4               # フェーズ4のドメインを調査
```
...
```bash
/brief-validate-phase 2               # フェーズ2のテストカバレッジを監査
```
...
```bash
/brief-resume-work                    # コンテキストリセットまたは新しいセッション後に使用
```
...
```bash
/brief-pause-work                     # continue-here.mdを作成
```
...
```bash
/brief-manager                        # コ��ンドセンターダッシュボードを開く
```
...
```bash
/brief-analyze-dependencies            # 依存関係の分析と提案
```
...
```bash
/brief-quick                          # 基本的なクイックタスク
/brief-quick --discuss --research     # ディスカッション＋調査＋計画
/brief-quick --full                   # プランチェックと検証付き
/brief-quick --discuss --research --full  # すべてのオプションステージ
```
...
```bash
/brief-autonomous                     # 残りの全フェーズを実行
/brief-autonomous --from 3            # フェーズ3から開始
/brief-autonomous --to 5              # フェーズ5まで実行
/brief-autonomous --from 3 --to 5     # フェーズ3〜5の範囲を実行
/brief-autonomous --only 4            # フェーズ4のみを自律実行
```

### `/brief-do`

フリーテキストを適切なBRIEFコマンドにルーティングします。

```bash
/brief-do                             # その後、やりたいことを説明
```
...
/brief-debug --diagnose "API returning 500 on /users endpoint"
```
...
```bash
/brief-add-tests 2                    # フェーズ2のテストを生成
```
...
```bash
/brief-stats                          # プロジェクトメトリクスダッシュボード
```
...
- `/brief-dev-preferences` コマンド — 任意のセッションでプリファレンスをロード
- `CLAUDE.md` プロファイルセクション — Claude Codeが自動検出

```bash
/brief-profile-user                   # セッションを分析してプロファイルを構築
/brief-profile-user --questionnaire   # 対話型アンケートのフォールバック
/brief-profile-user --refresh         # 新鮮な分析からの再生成
```
...
```bash
/brief-health                         # 整合性チェック
/brief-health --repair                # チェックして修復
```
...
```bash
/brief-forensics                              # 対話型 — 問題の入力を促す
/brief-forensics "Phase 3 execution stalled"  # 問題の説明付き
```
...
**前提条件:** アクティブなBRIEFプロジェクト
**生成物:** `.planning/` 配下のワークストリームディレクトリ、ワークストリームごとの状態追跡

```bash
/brief-workstreams                    # すべてのワークストリームを一覧表示
/brief-workstreams create backend-api # 新しいワークストリームを作成
/brief-workstreams switch backend-api # アクティブなワークストリームを設定
/brief-workstreams status backend-api # 詳細ステータス
/brief-workstreams progress           # ワークストリーム横断の進捗概要
/brief-workstreams complete backend-api  # 完了したワークストリームをアーカイブ
/brief-workstreams resume backend-api    # ワークストリームでの作業を再開
```
...
```bash
/brief-set-profile budget             # budgetプロファイルに切り替え
/brief-set-profile quality            # qualityプロファイルに切り替え
```
...
```bash
/brief-map-codebase                   # コードベース全体を分析
/brief-map-codebase auth              # auth領域にフォーカス
```
...
変更履歴のプレビュー付きでBRIEFをアップデートします。

```bash
/brief-update                         # アップデートを確認してインストール
```

### `/brief-reapply-patches`

BRIEFアップデート後にローカルの変更を復元します。

```bash
/brief-reapply-patches                # ローカルの変更をマージバック
```
...
**`/brief-quick` の代替ではありません** — 調査、複数ステップの計画、または検証が必要な場合は `/brief-quick` を使用してください。
...
**生成物:** `{phase}-REVIEWS.md` — `/brief-plan-phase --reviews` で利用可能
...
**目的:** レビュアーにはコード変更のみを表示し、BRIEF計画アーティファクトは含めません。

```bash
/brief-pr-branch                     # mainに対してフィルタリング
/brief-pr-branch develop             # developに対してフィルタリング
```
...
**999.x番号付け**により、バックログ項目はアクティブなフェーズシーケンスの外に保持されます。フェーズディレクトリは即座に作成されるため、`/brief-discuss-phase` や `/brief-plan-phase` がそれらに対して動作します。
...
**利用先:** `/brief-new-milestone`（シードをスキャンしてマッチするものを提示）
...
スレッドは、複数のセッションにまたがるが特定のフェーズに属さない作業のための軽量なクロスセッション知識ストアです。`/brief-pause-work` よりも軽量です。

```bash
/brief-thread                         # すべてのスレッドを一覧表示
/brief-thread fix-deploy-key-auth     # スレッドを再開
/brief-thread "Investigate TCP timeout in pasta service"  # 新規作成
```
