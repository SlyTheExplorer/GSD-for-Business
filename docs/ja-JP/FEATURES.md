...
**コマンド:** `/brief-new-project [--auto @file.md]`
...
- 既存のコードベースコンテキスト（`/brief-map-codebase` から取得）がある場合は読み込む

---

### 2. フェーズディスカッション

**コマンド:** `/brief-discuss-phase [N] [--auto] [--batch]`
...
**コマンド:** `/brief-plan-phase [N] [--auto] [--skip-research] [--skip-verify]`
...
- REQ-PLAN-07: フロントエンドフェーズが検出され UI-SPEC.md が存在しない場合、システムはユーザーに `/brief-ui-phase` の実行を促さなければならない（UI セーフティゲート）
...
**コマンド:** `/brief-audit-milestone`、`/brief-complete-milestone`、`/brief-new-milestone [name]`
...
**ギャップクローズ:** `/brief-plan-milestone-gaps` は監査で特定されたギャップを埋めるためのフェーズを作成します。
...
**コマンド:** `/brief-add-phase`、`/brief-insert-phase [N]`、`/brief-remove-phase [N]`
...
**コマンド:** `/brief-quick [--full] [--discuss] [--research]`

**目的:** BRIEF の保証を維持しながら、より高速なパスでアドホックなタスクを実行します。
...
**目的:** 自由形式のテキストを分析し、適切な BRIEF コマンドにルーティングします。

**要件:**
- REQ-DO-01: システムは自然言語入力からユーザーの意図を解析しなければならない
- REQ-DO-02: システムは意図を最も適切な BRIEF コマンドにマッピングしなければならない
...
- REQ-NEXT-04: プロジェクトが存在しない場合、システムは `/brief-new-project` を提案しなければならない
- REQ-NEXT-05: すべてのフェーズが完了している場合、システムは `/brief-complete-milestone` を提案しなければならない

**状態検出ロジック:**
| 状態 | アクション |
|------|----------|
| `.planning/` ディレクトリなし | `/brief-new-project` を提案 |
| フェーズに CONTEXT.md がない | `/brief-discuss-phase` を実行 |
| フェーズに PLAN.md ファイルがない | `/brief-plan-phase` を実行 |
| プランはあるが SUMMARY.md がない | `/brief-execute-phase` を実行 |
| 実行済みだが VERIFICATION.md がない | `/brief-verify-work` を実行 |
| すべてのフェーズが完了 | `/brief-complete-milestone` を提案 |
...
- REQ-POSTVER-03: システムは `/brief-verify-work` が対処すべき問題をログに記録しなければならない
...
**実行タイミング:** `/brief-execute-phase` の検証ステップの前に自動実行されます。
...
**実行タイミング:** `/brief-plan-phase` の末尾、プランチェッカーループの後に自動実行されます。
...
- REQ-CTX-06: コンテキストモニターは BRIEF アクティブ vs 非 BRIEF アクティブプロジェクトを区別しなければならない
...
**コマンド:** `/brief-pause-work`、`/brief-resume-work`、`/brief-progress`
...
**コマンド:** `/brief-set-profile <quality|balanced|budget|inherit>`
...
**コマンド:** `/brief-map-codebase [area]`

**目的:** 新しいプロジェクトを開始する前に既存のコードベースを分析し、BRIEF が既存の構成を理解できるようにします。
...
- REQ-MAP-04: 後続の `/brief-new-project` はコードベースマッピングを読み込み、追加する内容に焦点を当てた質問を行わなければならない
...
**コマンド:** `/brief-add-todo [desc]`、`/brief-check-todos`
...
**目的:** BRIEF を最新バージョンに更新し、チェンジログのプレビューを表示します。
...
- REQ-UPDATE-05: `/brief-reapply-patches` は更新後にローカルの変更を復元しなければならない
...
**目的:** 複数の AI コーディングエージェントランタイムで BRIEF を実行します。
...
- REQ-RUNTIME-05: アンインストールは他の設定に影響を与えることなく、すべての BRIEF ファイルをクリーンに削除しなければならない
...
**コマンド:** `/brief-profile-user [--questionnaire] [--refresh]`
...
- `/brief-dev-preferences` コマンド — 任意のセッションで好みを読み込み
...
- `brief-user-profiler` エージェント — セッションデータからの行動分析
...
すべての `/brief-progress` 呼び出しで、現在のマイルストーンのすべてのフェーズの未解決項目（pending、skipped、blocked、human_needed）をスキャンします。アクション可能なリンク付きのノンブロッキング警告セクションを表示します。
...
**要件:**
- REQ-DEBT-01: システムは `/brief-progress` ですべての過去フェーズの未解決 UAT/検証項目を表面化しなければならない
...
- REQ-DEBT-06: `/brief-audit-uat` はすべてのフェーズをスキャンし、項目をテスト可能性別にカテゴリ分けし、人的テストプランを生成しなければならない
...
**コマンド:** `/brief-fast [task description]`
...
**`/brief-quick` との使い分け:**
- `/brief-fast` — 2分以内に実行可能な一文のタスク（タイポ修正、設定変更、小規模な追加）
- `/brief-quick` — リサーチ、マルチステッププランニング、または検証が必要なもの

---

### 42. クロス AI ピアレビュー

**コマンド:** `/brief-review --phase N [--gemini] [--claude] [--codex] [--coderabbit] [--opencode] [--qwen] [--cursor] [--all]`
...
- REQ-REVIEW-05: レビューは `/brief-plan-phase --reviews` で使用可能でなければならない
...
**コマンド:** `/brief-add-backlog <description>`、`/brief-review-backlog`、`/brief-plant-seed <idea>`
...
- REQ-BACKLOG-02: `/brief-discuss-phase` と `/brief-plan-phase` が動作するよう、フェーズディレクトリは即座に作成されなければならない
- REQ-BACKLOG-03: `/brief-review-backlog` は項目ごとにプロモート、維持、削除のアクションをサポートしなければならない
- REQ-BACKLOG-04: プロモートされた項目はアクティブなマイルストーンシーケンスに再番号付けされなければならない
- REQ-SEED-01: シードは完全な WHY と表面化条件の WHEN をキャプチャしなければならない
- REQ-SEED-02: `/brief-new-milestone` はシードをスキャンして一致するものを提示しなければならない
...
**コマンド:** `/brief-thread [name | description]`

**目的:** 複数セッションにまたがるが特定のフェーズには属さない作業のための、軽量なクロスセッションナレッジストア。`/brief-pause-work` よりも軽量 — フェーズ状態やプランコンテキストは不要です。
...
**コマンド:** `/brief-pr-branch [target branch]`

**目的:** `.planning/` のコミットを除外して、プルリクエストに適したクリーンなブランチを作成します。レビュアーにはコード変更のみが表示され、BRIEF プランニング成果物は表示されません。
...
**目的:** BRIEF のプランニング成果物に対する多層防御セキュリティ。BRIEF は LLM のシステムプロンプトとなるマークダウンファイルを生成するため、これらのファイルに流入するユーザー制御テキストは間接的なプロンプトインジェクションの潜在的なベクターです。
...
Claude が BRIEF ワークフローコンテキスト外でファイル編集を試行した際に検出する PreToolUse フック。直接編集の代わりに `/brief-quick` や `/brief-fast` の使用をアドバイスします。`hooks.workflow_guard`（デフォルト: false）で設定可能です。
...
**目的:** `/brief-discuss-phase` 中に `DISCUSSION-LOG.md` を自動生成し、ディスカッション中の決定事項の完全な監査証跡を残します。
...
**コマンド:** `/brief-forensics [description]`

**目的:** 失敗または停滞した BRIEF ワークフローのポストモーテム調査。
...
**コマンド:** `/brief-milestone-summary [version]`
...
2. **切り替え** — 後続の BRIEF コマンドのためにアクティブなワークストリームコンテキストを変更
...
**コマンド:** `/brief-discuss-phase`（`workflow.discuss_mode: 'assumptions'` 設定時）
...
**対象:** `/brief-new-project` および `/brief-progress`

**目的:** UI 重視のプロジェクトを自動検出し、`/brief-ui-phase` の推奨を表面化します。
...
- REQ-UI-DETECT-03: システムは UI 重視フェーズのネクストステップに `/brief-ui-phase` を提案しなければならない
- REQ-UI-DETECT-04: システムは `/brief-ui-phase` を必須にしてはならない

**プロセス:**
1. **検出** — プロジェクト説明と技術スタックの UI シグナル（キーワード、フレームワーク参照）をスキャン
2. **アノテーション** — ROADMAP.md の該当フェーズに `ui_hint` マーカーを追加
3. **表面化** — UI 重視フェーズのネクストステップに `/brief-ui-phase` の推奨を含める
...
3. **インストール** — 1回のセッションで選択されたすべてのランタイムに対して BRIEF を設定
...
2. **変換** — BRIEF プロンプトを Windsurf ルール形式に変換
3. **インストール** — Windsurf 設定ディレクトリに BRIEF を設定
...
**目的:** BRIEF ドキュメントをポルトガル語、韓国語、日本語で提供します。
...
**目的:** CLI セッションなしでプログラムから BRIEF ワークフローを実行するためのヘッドレス TypeScript SDK。

**要件:**
- REQ-SDK-01: SDK は BRIEF ワークフロー操作を TypeScript 関数として公開しなければならない
- REQ-SDK-02: SDK はインタラクティブプロンプトなしのヘッドレス実行をサポートしなければならない
- REQ-SDK-03: SDK は CLI 駆動ワークフローと同一のアーティファクトを生成しなければならない

**プロセス:**
1. **インポート** — BRIEF SDK を TypeScript/JavaScript プロジェクトにインポート
2. **設定** — プロジェクトパスとワークフローオプションをプログラムから設定
3. **実行** — API コールで BRIEF フェーズ（discuss、plan、execute）を実行
...
**コマンド:** `/brief-execute-phase` 実行時に自動
...
**コマンド:** `/brief-docs-update`

**目的:** 正確性チェック付きのプロジェクトドキュメントを生成・検証します。

**要件:**
- REQ-DOCS-01: システムはドキュメント生成のために `brief-doc-writer` エージェントを起動しなければならない
- REQ-DOCS-02: システムは正確性チェックのために `brief-doc-verifier` エージェントを起動しなければならない
...
**プロセス:**
1. **生成** — `brief-doc-writer` を起動して実装からドキュメントを作成・更新
2. **検証** — `brief-doc-verifier` を起動してコードベースに対するドキュメント正確性をチェック
...
**フラグ:** `/brief-discuss-phase <N> --chain`
...
**目的:** BRIEF コマンドを Claude Code 2.1.88+ のスキル形式に後方互換性を維持してマイグレーションします。

**要件:**
- REQ-SKILLS-01: インストーラーは Claude Code 2.1.88+ 向けに `skills/brief-*/SKILL.md` を書き込まなければならない
...
2. **マイグレーション** — 各 BRIEF コマンドに対して `skills/brief-*/SKILL.md` ファイルを書き込み
...
**対象:** `/brief-execute-phase`（ベリファイヤーステップ）
...
**コマンド:** `/brief-analyze-dependencies`

**目的:** フェーズ依存関係を検出し、`/brief-manager` 実行前に ROADMAP.md への `Depends on` エントリを提案します。
...
**目的:** Trae IDE、Cline、Augment Code ランタイムへの BRIEF インストールを拡張します。
