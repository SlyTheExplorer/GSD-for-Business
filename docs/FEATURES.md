...
- [v1.34.0 Features](#v1340-features)
  ...
  - [Hard Stop Safety Gates in /brief-next](#101-hard-stop-safety-gates-in-gsd-next)
...
- [v1.35.0 Features](#v1350-features)
  ...
  - [BRIEF-2 Reverse Migration](#105-gsd-2-reverse-migration)
...
**Command:** `/brief-new-project [--auto @file.md]`
...
- Existing codebase context (from `/brief-map-codebase`) is loaded if present

---

### 2. Phase Discussion

**Command:** `/brief-discuss-phase [N] [--auto] [--batch]`
...
**Command:** `/brief-plan-phase [N] [--auto] [--skip-research] [--skip-verify]`
...
- REQ-PLAN-07: System MUST prompt user to run `/brief-ui-phase` if frontend phase detected and no UI-SPEC.md exists (UI safety gate)
...
**Commands:** `/brief-audit-milestone`, `/brief-complete-milestone`, `/brief-new-milestone [name]`
...
**Gap Closure:** `/brief-plan-milestone-gaps` creates phases to close gaps identified by audit.
...
**Commands:** `/brief-add-phase`, `/brief-insert-phase [N]`, `/brief-remove-phase [N]`
...
**Command:** `/brief-quick [--full] [--discuss] [--research]`

**Purpose:** Ad-hoc task execution with BRIEF guarantees but a faster path.
...
**Command:** `/brief-autonomous [--from N]`
...
**Purpose:** Analyze freeform text and route to the appropriate BRIEF command.

**Requirements:**
- REQ-DO-01: System MUST parse user intent from natural language input
- REQ-DO-02: System MUST map intent to the best matching BRIEF command
...
- REQ-NEXT-04: System MUST suggest `/brief-new-project` if no project exists
- REQ-NEXT-05: System MUST suggest `/brief-complete-milestone` when all phases are complete

**State Detection Logic:**
| State | Action |
|-------|--------|
| No `.planning/` directory | Suggest `/brief-new-project` |
| Phase has no CONTEXT.md | Run `/brief-discuss-phase` |
| Phase has no PLAN.md files | Run `/brief-plan-phase` |
| Phase has plans but no SUMMARY.md | Run `/brief-execute-phase` |
| Phase executed but no VERIFICATION.md | Run `/brief-verify-work` |
| All phases complete | Suggest `/brief-complete-milestone` |
...
- REQ-NYQ-05: System MUST support retroactive validation via `/brief-validate-phase`
...
**Retroactive Validation (`/brief-validate-phase [N]`):**
...
- REQ-POSTVER-03: System MUST log issues for `/brief-verify-work` to address
...
**When:** Runs automatically during `/brief-execute-phase` before the verifier step.
...
**When:** Runs automatically at the end of `/brief-plan-phase` after the plan checker loop.
...
- REQ-CTX-06: Context monitor MUST differentiate BRIEF-active vs non-BRIEF-active projects
...
**Commands:** `/brief-pause-work`, `/brief-resume-work`, `/brief-progress`
...
**Command:** `/brief-set-profile <quality|balanced|budget|inherit>`
...
**Command:** `/brief-map-codebase [area]`

**Purpose:** Analyze an existing codebase before starting a new project, so BRIEF understands what exists.
...
- REQ-MAP-04: Subsequent `/brief-new-project` MUST load codebase mapping and focus questions on what's being added
...
**Command:** `/brief-debug [description]`
...
**Commands:** `/brief-add-todo [desc]`, `/brief-check-todos`
...
**Purpose:** Update BRIEF to the latest version with changelog preview.
...
- REQ-UPDATE-05: `/brief-reapply-patches` MUST restore local modifications after update

---

### 32. Settings Management

**Command:** `/brief-settings`
...
**Purpose:** Run BRIEF across multiple AI coding agent runtimes.
...
- REQ-RUNTIME-05: Uninstall MUST cleanly remove all BRIEF files without affecting other configurations
...
**Statusline Display:**
```
[⬆ /brief-update │] model │ [current task │] directory [█████░░░░░ 50%]
```
...
**Command:** `/brief-profile-user [--questionnaire] [--refresh]`
...
- `brief-user-profiler` agent — Behavioral analysis from session data
...
Every `/brief-progress` call scans ALL phases in the current milestone for outstanding items (pending, skipped, blocked, human_needed). Displays a non-blocking warning section with actionable links.
...
**Requirements:**
- REQ-DEBT-01: System MUST surface outstanding UAT/verification items from ALL prior phases in `/brief-progress`
...
- REQ-DEBT-06: `/brief-audit-uat` MUST scan all phases, categorize items by testability, and produce a human test plan
...
**Command:** `/brief-fast [task description]`
...
**When to use vs `/brief-quick`:**
- `/brief-fast` — One-sentence tasks executable in under 2 minutes (typo, config change, small addition)
- `/brief-quick` — Anything needing research, multi-step planning, or verification

---

### 42. Cross-AI Peer Review

**Command:** `/brief-review --phase N [--gemini] [--claude] [--codex] [--coderabbit] [--opencode] [--qwen] [--cursor] [--all]`
...
- REQ-REVIEW-05: Reviews MUST be consumable by `/brief-plan-phase --reviews`
...
**Commands:** `/brief-add-backlog <description>`, `/brief-review-backlog`, `/brief-plant-seed <idea>`
...
- REQ-BACKLOG-02: Phase directories MUST be created immediately so `/brief-discuss-phase` and `/brief-plan-phase` work on them
- REQ-BACKLOG-03: `/brief-review-backlog` MUST support promote, keep, and remove actions per item
...
- REQ-SEED-02: `/brief-new-milestone` MUST scan seeds and present matches
...
**Command:** `/brief-thread [name | description]`

**Purpose:** Lightweight cross-session knowledge stores for work that spans multiple sessions but doesn't belong to any specific phase. Lighter weight than `/brief-pause-work` — no phase state, no plan context.
...
**Command:** `/brief-pr-branch [target branch]`

**Purpose:** Create a clean branch suitable for pull requests by filtering out `.planning/` commits. Reviewers see only code changes, not BRIEF planning artifacts.
...
**Purpose:** Defense-in-depth security for BRIEF's planning artifacts. Because BRIEF generates markdown files that become LLM system prompts, user-controlled text flowing into these files is a potential indirect prompt injection vector.
...
PreToolUse hook that detects when Claude attempts file edits outside a BRIEF workflow context. Advises using `/brief-quick` or `/brief-fast` instead of direct edits. Configurable via `hooks.workflow_guard` (default: false).
...
**Purpose:** Auto-generate `DISCUSSION-LOG.md` during `/brief-discuss-phase` for full audit trail of decisions made during discussion.
...
**Command:** `/brief-forensics [description]`

**Purpose:** Post-mortem investigation of failed or stuck BRIEF workflows.
...
**Command:** `/brief-milestone-summary [version]`
...
2. **Switch** — Change active workstream context for subsequent BRIEF commands
...
**Command:** `/brief-discuss-phase` with `workflow.discuss_mode: 'assumptions'`
...
**Part of:** `/brief-new-project` and `/brief-progress`

**Purpose:** Automatically detect UI-heavy projects and surface `/brief-ui-phase` recommendation.
...
- REQ-UI-DETECT-03: System MUST suggest `/brief-ui-phase` in next steps for UI-heavy phases
- REQ-UI-DETECT-04: System MUST NOT make `/brief-ui-phase` mandatory
...
3. **Surface** — Include `/brief-ui-phase` recommendation in next steps for UI-heavy phases

---

### 55. Multi-Runtime Installer Selection

**Part of:** `npx brief-cc`
...
3. **Install** — Configure BRIEF for all selected runtimes in a single session
...
**Purpose:** Add Windsurf as a supported AI CLI runtime for BRIEF installation and execution.

**Requirements:**
- REQ-WINDSURF-01: Installer MUST detect Windsurf runtime and offer it as a target
- REQ-WINDSURF-02: BRIEF commands MUST function correctly within Windsurf sessions

**Process:**
1. **Detect** — Identify Windsurf runtime availability on the system
2. **Install** — Configure BRIEF skills and hooks for the Windsurf environment
...
**Purpose:** Provide BRIEF documentation in Portuguese, Korean, and Japanese.
...
**Purpose:** Headless TypeScript SDK for running BRIEF workflows programmatically without a CLI session.

**Requirements:**
- REQ-SDK-01: SDK MUST expose BRIEF workflow operations as TypeScript functions
...
**Process:**
1. **Import** — Import BRIEF SDK into a TypeScript/JavaScript project
2. **Configure** — Set project path and workflow options programmatically
3. **Execute** — Run BRIEF phases (discuss, plan, execute) via API calls
...
**Command:** Automatic during `/brief-execute-phase`
...
**Command:** `/brief-docs-update`

**Purpose:** Generate and verify project documentation with accuracy checks.

**Requirements:**
- REQ-DOCS-01: System MUST spawn `brief-doc-writer` agent to generate documentation
- REQ-DOCS-02: System MUST spawn `brief-doc-verifier` agent to check accuracy
...
**Process:**
1. **Generate** — Spawn `brief-doc-writer` to create or update documentation from implementation
2. **Verify** — Spawn `brief-doc-verifier` to check documentation accuracy against codebase
...
**Flag:** `/brief-discuss-phase <N> --chain`
...
**Purpose:** Migrate BRIEF commands to Claude Code 2.1.88+ skills format with backward compatibility.

**Requirements:**
- REQ-SKILLS-01: Installer MUST write `skills/brief-*/SKILL.md` for Claude Code 2.1.88+
...
2. **Migrate** — Write `skills/brief-*/SKILL.md` files for each BRIEF command
...
**Part of:** `/brief-execute-phase` (verifier step)
...
**Part of:** BRIEF SDK prompt assembly
...
**Flag:** `/brief-discuss-phase --power`
...
**Command:** `/brief-analyze-dependencies`

**Purpose:** Detect phase dependencies and suggest `Depends on` entries for ROADMAP.md before running `/brief-manager`.
...
**Part of:** `/brief-verify-work` (optional)
...
**Purpose:** Extend BRIEF installation to Trae IDE, Cline, and Augment Code runtimes.
...
**Flag:** `/brief-autonomous --interactive`
...
**Purpose:** Optional git and session hooks for BRIEF projects, gated behind `hooks.community: true` in config.
...
**Command:** `/brief-intel [query <term>|status|diff|refresh]`
...
**References:** `brief/references/gates.md`
...
**Commands:** `/brief-code-review`, `/brief-code-review-fix`
...
**Command:** `/brief-explore [topic]`

**Purpose:** Guide a developer through exploring an idea via Socratic probing questions before committing to a plan. Routes outputs to the appropriate BRIEF artifact: notes, todos, seeds, research questions, requirements updates, or a new phase.

**Requirements:**
- REQ-EXPLORE-01: Exploration MUST use Socratic probing — ask questions before proposing solutions
- REQ-EXPLORE-02: Session MUST offer to route outputs to the appropriate BRIEF artifact
...
**Command:** `/brief-undo --last N | --phase NN | --plan NN-MM`

**Purpose:** Roll back BRIEF phase or plan commits safely using the phase manifest and git log, with dependency checks and a hard confirmation gate before any revert is applied.
...
- REQ-UNDO-03: `--last N` mode MUST display recent BRIEF commits for interactive selection
...
**Command:** `/brief-import --from <filepath>`

**Purpose:** Ingest an external plan file into the BRIEF planning system with conflict detection against `PROJECT.md` decisions, converting it to a valid BRIEF PLAN.md and validating it through the plan-checker.
...
- REQ-IMPORT-03: Imported plan MUST be written as a valid BRIEF PLAN.md format
- REQ-IMPORT-04: Written plan MUST pass `brief-plan-checker` validation

---

### 97. Rapid Codebase Scan

**Command:** `/brief-scan [--focus tech|arch|quality|concerns|tech+arch]`

**Purpose:** Lightweight alternative to `/brief-map-codebase` that spawns a single mapper agent for a specific focus area, producing targeted output in `.planning/codebase/` without the overhead of 4 parallel agents.
...
- REQ-SCAN-03: Output MUST be written to `.planning/codebase/` in the same format as `/brief-map-codebase`

---

### 98. Autonomous Audit-to-Fix

**Command:** `/brief-audit-fix [--source <audit>] [--severity high|medium|all] [--max N] [--dry-run]`
...
### 101. Hard Stop Safety Gates in /brief-next

**Command:** `/brief-next`

**Purpose:** Prevent `/brief-next` from entering runaway loops by adding hard stop safety gates and a consecutive-call guard that interrupts autonomous chaining when repeated identical steps are detected.

**Requirements:**
- REQ-NEXT-GATE-01: `/brief-next` MUST track consecutive same-step calls
...
- REQ-ADAPTIVE-02: `adaptive` MUST be selectable via `/brief-set-profile adaptive`

---

### 103. Post-Merge Hunk Verification

**Command:** `/brief-reapply-patches`
...
**Purpose:** Extend BRIEF installation to Cline, CodeBuddy, and Qwen Code runtimes.
...
- REQ-CODEBUDDY-01: CodeBuddy install MUST deploy skills to `~/.codebuddy/skills/brief-*/SKILL.md`. Flag: `--codebuddy`.
- REQ-QWEN-01: Qwen Code install MUST deploy skills to `~/.qwen/skills/brief-*/SKILL.md`, following the open standard used by Claude Code 2.1.88+. `QWEN_CONFIG_DIR` env var overrides the default path. Flag: `--qwen`.
...
### 105. BRIEF-2 Reverse Migration

**Command:** `/brief-from-gsd2 [--dry-run] [--force] [--path <dir>]`

**Purpose:** Migrate a project from BRIEF-2 format (`.gsd/` directory with Milestone→Slice→Task hierarchy) back to the v1 `.planning/` format, restoring full compatibility with all BRIEF v1 commands.
...
| `--path <dir>` | Specify the BRIEF-2 root directory |

---

### 106. AI Integration Phase Wizard

**Command:** `/brief-ai-integration-phase [N]`
...
**Command:** `/brief-plan-phase N --bounce`
...
**Command:** `/brief-ship` (enhanced)

**Purpose:** Before the manual review step in `/brief-ship`, automatically run an external code review command if configured. The command receives the diff and phase context via stdin and returns a JSON verdict (`APPROVED` or `REVISE`). Falls through to the existing manual review flow regardless of outcome.
...
**Command:** `/brief-execute-phase N --cross-ai`
...
**Command:** `/brief-plan-phase` (enhanced research step)
...
**Command:** `/brief-extract-learnings N`
...
**Purpose:** Allow projects to store their CLAUDE.md in a non-root location. The `claude_md_path` config key controls where `/brief-profile-user` and related commands write the generated CLAUDE.md file.
