...
| Wave-based 4-cap actually fires at runtime (DSC-03 runtime parallelism smoke) | DSC-03 | Live Task spawn cannot be exercised in node:test (orchestrator-layer concern) | Before first `/brief-discover` run in production, executor MUST spawn a 2-task smoke (2 dummy researchers from the same orchestrator message). If wall-clock time ≈ single-researcher time (parallel execution), proceed with 4-wide. If wall-clock ≈ 2× single-researcher time (sequential serialization), fall back to sequential execution and flag HIGH per CONTEXT Risk Note line 275-276. For full flow validation: run `/brief-discover` with 5 categories selected; verify orchestrator message contains exactly 4 Task blocks for wave 1 and 1 Task block for wave 2; verify wall-clock for wave 1 < 4× single-researcher wall-clock × 1.1 |
