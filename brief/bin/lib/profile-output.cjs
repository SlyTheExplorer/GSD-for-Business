...
const CLAUDE_MD_FALLBACKS = {
  project: 'Project not yet initialized. Run /brief-new-project to set up.',
...
function cmdWriteProfile(cwd, options, raw) {
  ...
  if (!outputPath) {
    outputPath = path.join(os.homedir(), '.claude', 'brief', 'USER-PROFILE.md');
...
function cmdGenerateDevPreferences(cwd, options, raw) {
  ...
  if (analysis.data_source === 'questionnaire') {
    stackBlock = 'Stack preferences not available (questionnaire-only profile). Run `/brief-profile-user --refresh` with session data to populate.';
  ...
  const result = {
    ...
    command_name: '/brief-dev-preferences',
...
function cmdGenerateClaudeProfile(cwd, options, raw) {
  ...
  const sectionLines = [
    ...
    '## Developer Profile',
    '',
    `> Generated by GSD from ${dataSource}. Run \`/brief-profile-user --refresh\` to update.`,
    '',
...
function cmdGenerateClaudeMd(cwd, options, raw) {
  ...
  if (profileStatus === 'placeholder_added') message += ' Run /brief-profile-user to unlock Developer Profile.';
