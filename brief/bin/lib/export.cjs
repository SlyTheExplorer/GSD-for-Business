 ...
 * @param {string|null} [options._gate=null]           DEV-MODE ESCAPE. When set to 'audience' | 'compliance' | 'both' (default null = run all 3 gates: ALIGN→AUDIENCE→COMPLIANCE per RESEARCH.md Open Questions §1 RESOLVED), restricts which gate(s) run. Used by `brief-tools export run --gate <name>` for targeted CLI testing. Production workflow leaves this undefined → all gates run.
