...
> **Note:** Planning methodology is in `agents/brief-planner.md`.
...
<!-- For checkpoint task examples and patterns, see @~/.claude/brief/references/checkpoints.md -->
...
**Wave is pre-computed:** Wave numbers are assigned during `/brief-plan-phase`. Execute-phase reads `wave` directly from frontmatter and groups plans by wave number. No runtime dependency analysis needed.
...
See `~/.claude/brief/references/tdd.md` for TDD plan structure.
...
See `~/.claude/brief/templates/user-setup.md` for full schema and examples
...
See `~/.claude/brief/workflows/verify-phase.md` for verification logic.
