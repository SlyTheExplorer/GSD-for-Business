...
Template for project-root `CLAUDE.md` — auto-generated by `brief-tools generate-claude-md`.
...
**Fallback text:**
```
Project not yet initialized. Run /brief-new-project to set up.
```

### Stack Section
```
<!-- BRIEF:stack-start source:STACK.md -->
## Technology Stack
...
- Skips BRIEF's own installed skills (directories starting with `gsd-`)
...
6. **Workflow Enforcement** — Default BRIEF entry points for file-changing work
7. **Profile** — Developer behavioral preferences (how to interact)

## Marker Format

- Start: `<!-- BRIEF:{name}-start source:{file} -->`
- End: `<!-- BRIEF:{name}-end -->`
