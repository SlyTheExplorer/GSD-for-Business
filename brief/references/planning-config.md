...
| `manager.flags.discuss` | `""` | Flags passed to `/brief-discuss-phase` when dispatched from manager (e.g. `"--auto --analyze"`) |
...
**Using brief-tools.cjs (preferred):**
...
**Note:** Most BRIEF operations use direct file reads or explicit paths, which work regardless of gitignore status.
...
| `review.models.<cli>` | string\|null | `null` | Any model ID string | Per-CLI model override for /brief-review (e.g., `review.models.gemini`). Falls back to CLI default when null. |
...
| `workflow.ai_integration_phase` | boolean | `true` | `true`, `false` | Run /brief-ai-integration-phase before planning AI system phases |
...
Set via `intel.*` namespace (e.g., `"intel": { "enabled": true }`). Controls the queryable codebase intelligence system consumed by `/brief-intel`.

| Key | Type | Default | Allowed Values | Description |
|-----|------|---------|----------------|-------------|
| `intel.enabled` | boolean | `false` | `true`, `false` | Enable queryable codebase intelligence system. When `true`, `/brief-intel` commands build and query a JSON index in `.planning/intel/`. |
...
| Key | Type | Default | Allowed Values | Description |
|-----|------|---------|----------------|-------------|
| `manager.flags.discuss` | string | `""` | Any CLI flags string | Flags passed to `/brief-discuss-phase` from manager (e.g., `"--auto --analyze"`) |
...
8. **`sub_repos` auto-sync** -- On every config load, BRIEF scans for child directories with `.git` and updates the `sub_repos` array if the filesystem has changed. Legacy `multiRepo: true` is automatically migrated to a detected `sub_repos` array.
