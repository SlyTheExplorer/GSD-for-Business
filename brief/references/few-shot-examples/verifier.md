...
**Input:** Must-have artifact: `brief/workflows/context-bridge.md: >50 lines`

**Output:**

> **L1 (Existence):** EXISTS -- `brief/workflows/context-bridge.md` found, 187 lines.
...
> **L3 (Wired):** WIRED -- Referenced by `brief/workflows/execute-phase.md` at step 3. Called by `agents/brief-executor.md` via @-reference include. Arguments verified: receives planning directory path matching the expected parameter.
> **Status: PASSED**
...
> **Method:** Ran `grep -n "scope" brief/bin/validate-commit.js` to locate scope validation logic.
...
> **Ran:** `echo "feat(: test" | node brief/bin/validate-commit.js` -- exit code 0 (accepted).
...
> **Method:** Ran `grep -n "feedback-loop\|extractFeedback" brief/bin/check-subagent-output.js`.
...
> **Cross-check:** `grep -rn "require.*feedback-loop" brief/bin/` returns zero results across all scripts.
...
> **Method:** Ran `grep "architecture_graph\|architecture_guard" brief/bin/lib/config-schema.json`.
> **Evidence:** Zero matches in schema. However, `grep "architecture" brief/bin/lib/config.cjs` shows both flags in CONFIG_DEFAULTS (lines 426, 428).
...
**Input:** Must-have artifact: `brief/bin/lib/audit-checks/index.cjs: >100 lines`

**Output:**

> **L1 (Existence):** EXISTS -- File found at `brief/bin/lib/audit-checks/index.cjs`, 142 lines.
> **Status: PASSED**
