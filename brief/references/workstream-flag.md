...
The `--ws <name>` flag scopes BRIEF operations to a specific workstream, enabling
...
BRIEF now prefers a session-scoped pointer keyed by runtime/session identity
...
When BRIEF resolves the session-scoped pointer in step 3 above, it uses this order:
...
If none of those produce a stable identity, BRIEF does not keep probing. It falls
...
non-interactive, BRIEF intentionally skips shelling out to `tty` because that path
...
- If that was the last pointer for the repo, BRIEF also removes the now-empty
  per-project temp directory
- If sibling session pointers still exist, the temp directory is left in place
- When a pointer refers to a workstream directory that no longer exists, BRIEF
...
BRIEF does not currently run a background garbage collector for historical temp
