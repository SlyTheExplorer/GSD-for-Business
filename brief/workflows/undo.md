<purpose>
Safe git revert workflow. Rolls back BRIEF phase or plan commits using the phase manifest with dependency checks and a confirmation gate. Uses git revert --no-commit (NEVER git reset) to preserve history.
</purpose>
...
```
Usage: /brief-undo --last N | --phase NN | --plan NN-MM

Modes:
  --last N      Show last N BRIEF commits for interactive selection
...
Filter for BRIEF conventional commits matching `type(scope): message` pattern (e.g., `feat(04-01):`, `docs(03):`, `fix(02-03):`).

Display a numbered list of matching commits:
```
Recent BRIEF commits:
...
```
Working tree has uncommitted changes. Commit or stash them before running /brief-undo.
```
...
**Also available:**
- `/brief-execute-phase ${PHASE}` — re-execute if needed
- `/brief-undo --last 1` — undo the revert itself if something went wrong
