...
```bash
INIT=$(node "$HOME/.claude/brief/bin/brief-tools.cjs" init phase-op "${PHASE}")
...
If "Skip": Exit with message: "Existing SPEC.md unchanged. Run /brief-discuss-phase [X] to continue."
...
Use the SPEC.md template from @~/.claude/brief/templates/spec.md.
...
- User directed to /brief-discuss-phase as next step
</success_criteria>
