...
There is no `/brief-transition` command. This workflow is invoked automatically by
`execute-phase` during auto-advance, or inline by the orchestrator after phase
verification. Users should never be told to run `/brief-transition`.

**Valid user commands for phase progression:**
- `/brief-discuss-phase {N}` — discuss a phase before planning
- `/brief-plan-phase {N}` — plan a phase
- `/brief-execute-phase {N}` — execute a phase
- `/brief-progress` — see roadmap progress
...
These will carry forward as debt. Review: `/brief-audit-uat`
```
...
Exit skill and invoke SlashCommand("/brief-plan-phase [X+1] --auto ${GSD_WS}")
...
Exit skill and invoke SlashCommand("/brief-discuss-phase [X+1] --auto ${GSD_WS}")
...
`/brief-discuss-phase [X+1] ${GSD_WS}` — gather context and clarify approach

---

**Also available:**
- `/brief-plan-phase [X+1] ${GSD_WS}` — skip discussion, plan directly
- `/brief-research-phase [X+1] ${GSD_WS}` — investigate unknowns
...
`/brief-plan-phase [X+1] ${GSD_WS}`

---

**Also available:**
- `/brief-discuss-phase [X+1] ${GSD_WS}` — revisit context
- `/brief-research-phase [X+1] ${GSD_WS}` — investigate unknowns
...
`/brief-workstreams complete {current_ws_name} ${GSD_WS}`

See overall milestone progress:

`/brief-workstreams progress ${GSD_WS}`
...
Do NOT suggest `/brief-complete-milestone` or `/brief-new-milestone`.
...
Exit skill and invoke SlashCommand("/brief-complete-milestone {version} ${GSD_WS}")
...
`/brief-complete-milestone {version} ${GSD_WS}`
