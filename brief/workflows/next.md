<purpose>
Detect current project state and automatically advance to the next logical BRIEF workflow step.
...
```
No BRIEF project detected. Run `/brief-new-project` to get started.
```
Exit.
</step>
...
Run `/brief-health` to diagnose, or manually fix STATE.md.
Use `--force` to bypass this check.
```
Exit.
...
**Source phase:** {src}
**Deferred at:** {date} during /brief-next advancement to Phase {dest}
**Plans:**
...
→ Next action: `/brief-discuss-phase <first-phase>`
...
▶ **Next step:** `/brief-[command] [args]`
  [One-line explanation of why this is the next step]
```

Then immediately invoke the determined command via SlashCommand.
Do not ask for confirmation — the whole point of `/brief-next` is zero-friction advancement.
</step>
