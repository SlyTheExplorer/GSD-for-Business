<purpose>
Interactive configuration of BRIEF workflow agents (research, plan_check, verifier) and model profile selection via multi-question prompt. Updates .planning/config.json with user preferences. Optionally saves settings as global defaults (~/.gsd/defaults.json) for future projects.
</purpose>
...
GSD_CONFIG_PATH=$(node "$HOME/.claude/brief/bin/brief-tools.cjs" config-path)
...
- `workflow.ui_safety_gate` — prompt to run /brief-ui-phase before planning frontend phases (default: true if absent)
...
AskUserQuestion([
  ...
  {
    question: "Enable UI Safety Gate? (prompts to run /brief-ui-phase before planning frontend phases)",
    header: "UI Gate",
    multiSelect: false,
    options: [
      { label: "Yes (Recommended)", description: "plan-phase asks to run /brief-ui-phase first when frontend indicators detected." },
  ...
  },
  {
    ...
    multiSelect: false,
    options: [
      { label: "Yes (Recommended)", description: "Run /brief-ai-phase before planning AI system phases. Surfaces the right framework, researches its docs, and designs the evaluation strategy." },
  ...
  },
  {
    ...
    options: [
      ...
      { label: "Yes", description: "Skip discuss in /brief-autonomous — chain directly to plan. Best for backend/pipeline work where phase descriptions are the spec." }
    ]
  },
  {
...
These settings apply to future /brief-plan-phase and /brief-execute-phase runs.

Quick commands:
- /brief-set-profile <profile> — switch model profile
- /brief-plan-phase --research — force research
- /brief-plan-phase --skip-research — skip research
- /brief-plan-phase --skip-verify — skip plan check
```
</step>
