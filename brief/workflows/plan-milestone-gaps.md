<purpose>
Create all phases necessary to close gaps identified by `/brief-audit-milestone`. Reads MILESTONE-AUDIT.md, groups gaps into logical phases, creates phase entries in ROADMAP.md, and offers to plan each phase. One command creates all fix phases — no manual `/brief-add-phase` per gap.
</purpose>
...
```
No audit gaps found. Run `/brief-audit-milestone` first.
```
...
**Also available:**
- `/brief-execute-phase {N}` — if plans already exist
...
`/brief-audit-milestone` — re-audit to verify gaps closed
`/brief-complete-milestone {version}` — archive when audit passes
```
...
- [ ] User knows to run `/brief-plan-phase` next
</success_criteria>
