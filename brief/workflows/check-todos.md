...
Todos are captured during work sessions with /brief-add-todo.

---

Would you like to:

1. Continue with current phase (/brief-progress)
2. Add a todo now (/brief-add-todo)
```
...
- `/brief-check-todos` → show all
- `/brief-check-todos api` → filter to area:api only
</step>
...
- `/brief-check-todos [area]` to filter by area
- `q` to exit
```
...
- options:
  - "Work on it now" — move to done, start working
  - "Create a phase" — /brief-add-phase with this scope
  - "Brainstorm approach" — think through before deciding
  - "Put it back" — return to list
</step>
...
**Create a phase:**
Display: `/brief-add-phase [description from todo]`
