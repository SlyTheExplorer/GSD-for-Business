...
```
No threads found. Create one with: /brief-thread <description>
```
...
2. Read the file and display a summary:
   ...
   ─────────────────────────────────────
   Resume with: /brief-thread {SLUG}
   Close with:  /brief-thread close {SLUG}
   ```
...
6. Report:
   ...
   Resume anytime with: /brief-thread {slug}
   Close when done with: /brief-thread close {slug}
   ```
</mode_create>
...
- Lighter weight than /brief-pause-work — no phase state, no plan context
- The value is in Context and Next Steps — a cold-start session can pick up immediately
- Threads can be promoted to phases or backlog items when they mature:
  /brief-add-phase or /brief-add-backlog with context from the thread
- Thread files live in .planning/threads/ — no collision with phases or other BRIEF structures
...
- The generate-slug call for new threads runs through gsd-sdk query (or brief-tools) which sanitizes input — keep that pattern
</security_notes>
