    ...
    if (fs.existsSync(cacheFile)) {
      try {
        ...
        if (cache.update_available) {
          gsdUpdate = '\x1b[33m⬆ /brief-update\x1b[0m │ ';
        }
        if (cache.stale_hooks && cache.stale_hooks.length > 0) {
          // If installed version is ahead of npm latest, this is a dev install.
          // Running /brief-update would downgrade — show a contextual warning instead.
          const isDevInstall = (() => {
          ...
          if (isDevInstall) {
            ...
            gsdUpdate += '\x1b[31m⚠ stale hooks — run /brief-update\x1b[0m │ ';
          }
        }
      } catch (e) {}
    }
