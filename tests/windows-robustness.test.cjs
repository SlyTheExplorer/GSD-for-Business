 ...
 *
 * Regression tests for: https://github.com/brief-build/brief/issues/1343
 */
...
describe('@file: handoff in workflows', () => {
  test('all workflows calling brief-tools init have @file: handler', () => {
    const workflowFiles = fs.readdirSync(WORKFLOWS_DIR)
      .filter(f => f.endsWith('.md'));
    ...
    for (const wf of workflowFiles) {
      ...
      // Check if this workflow calls brief-tools.cjs init
      if (/INIT=\$\(node.*brief-tools.*\binit\b/.test(content)) {
    ...
    assert.strictEqual(
      missing.length, 0,
      `Workflows calling brief-tools init without @file: handler (large output will be truncated):\n  ${missing.join('\n  ')}`
    );
  });
});
