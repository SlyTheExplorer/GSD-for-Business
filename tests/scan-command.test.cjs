...
describe('scan command', () => {
  ...
  test('workflow file exists', () => {
    const p = path.join(__dirname, '..', 'brief', 'workflows', 'scan.md');
    assert.ok(fs.existsSync(p), 'brief/workflows/scan.md should exist');
  });
  ...
  test('workflow references brief-codebase-mapper', () => {
    const p = path.join(__dirname, '..', 'brief', 'workflows', 'scan.md');
    const content = fs.readFileSync(p, 'utf-8');
    assert.ok(content.includes('brief-codebase-mapper'),
      'Workflow should reference the brief-codebase-mapper agent');
  });
});
