...
} = require('../brief/bin/lib/model-profiles.cjs');
...
describe('MODEL_PROFILES', () => {
  test('contains all expected GSD agents', () => {
    const expectedAgents = [
      'brief-planner', 'brief-roadmapper', 'brief-executor',
      'brief-phase-researcher', 'brief-project-researcher', 'brief-research-synthesizer',
      'brief-codebase-mapper', 'brief-verifier',
      'brief-plan-checker', 'brief-nyquist-auditor',
    ];
...
describe('VALID_PROFILES', () => {
  ...
  test('is derived from MODEL_PROFILES keys', () => {
    const fromData = Object.keys(MODEL_PROFILES['brief-planner']);
...
describe('getAgentToModelMapForProfile', () => {
  test('returns correct models for balanced profile', () => {
    ...
    assert.strictEqual(map['brief-planner'], 'opus');
    assert.strictEqual(map['brief-codebase-mapper'], 'haiku');
    assert.strictEqual(map['brief-verifier'], 'sonnet');
  });

  test('returns correct models for budget profile', () => {
    const map = getAgentToModelMapForProfile('budget');
    assert.strictEqual(map['brief-planner'], 'sonnet');
    assert.strictEqual(map['brief-phase-researcher'], 'haiku');
  });

  test('returns correct models for quality profile', () => {
    const map = getAgentToModelMapForProfile('quality');
    assert.strictEqual(map['brief-planner'], 'opus');
    assert.strictEqual(map['brief-executor'], 'opus');
  });

  test('returns correct models for adaptive profile', () => {
    const map = getAgentToModelMapForProfile('adaptive');
    assert.strictEqual(map['brief-planner'], 'opus', 'planner should use opus in adaptive');
    assert.strictEqual(map['brief-executor'], 'sonnet', 'executor should use sonnet in adaptive');
    assert.strictEqual(map['brief-codebase-mapper'], 'haiku', 'mapper should use haiku in adaptive');
    assert.strictEqual(map['brief-plan-checker'], 'haiku', 'checker should use haiku in adaptive');
  });
...
describe('formatAgentToModelMapAsTable', () => {
  test('produces a table with header and separator', () => {
    const map = { 'brief-planner': 'opus', 'brief-executor': 'sonnet' };
    ...
    assert.ok(table.includes('brief-planner'), 'should list agent');
