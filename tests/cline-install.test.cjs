 ...
 * completely absent from bin/install.js. Running `npx brief-cc`
 ...
 * - Installs brief/ engine with path replacement
 */
...
describe('Cline markdown conversion', () => {
  ...
  test('replaces .claude/ paths with .cline/', () => {
    const result = convertClaudeToCliineMarkdown('See ~/.claude/brief/');
  ...
  test('replaces .claude/skills/ with .cline/skills/', () => {
    const result = convertClaudeToCliineMarkdown('skills at .claude/skills/brief-executor');
...
describe('Cline install (local)', () => {
  ...
  test('install creates brief engine directory', () => {
    install(false, 'cline');
    const engineDir = path.join(tmpDir, 'brief');
    assert.ok(fs.existsSync(engineDir), 'brief directory must exist after install');
  });
    ...
    function scanDir(dir) {
      ...
      for (const entry of entries) {
        ...
        if (entry.isDirectory()) {
          ...
          const hasLeaked = /~\/\.claude\/(?:brief|commands|agents|hooks)|HOME\/\.claude\/(?:brief|commands|agents|hooks)/.test(content);
