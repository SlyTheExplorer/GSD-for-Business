...
const REFS_DIR = path.join(__dirname, '..', 'brief', 'references', 'few-shot-examples');
...
describe('few-shot calibration examples', () => {
  ...
  describe('agent files reference few-shot examples', () => {
    test('brief-plan-checker.md contains reference to plan-checker few-shot examples', () => {
      const content = readFile(path.join(AGENTS_DIR, 'brief-plan-checker.md'));
      assert.match(content, /@~\/\.claude\/brief\/references\/few-shot-examples\/plan-checker\.md/);
    });

    test('brief-verifier.md contains reference to verifier few-shot examples', () => {
      const content = readFile(path.join(AGENTS_DIR, 'brief-verifier.md'));
      assert.match(content, /@~\/\.claude\/brief\/references\/few-shot-examples\/verifier\.md/);
    });
  });
