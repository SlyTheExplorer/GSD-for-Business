...
describe('hook file validation', () => {
  test('brief-session-state.sh exists', () => {
    const hookPath = path.join(HOOKS_DIR, 'brief-session-state.sh');
    assert.ok(fs.existsSync(hookPath), 'brief-session-state.sh should exist');
  });

  test('brief-validate-commit.sh exists', () => {
    const hookPath = path.join(HOOKS_DIR, 'brief-validate-commit.sh');
    assert.ok(fs.existsSync(hookPath), 'brief-validate-commit.sh should exist');
  });

  test('brief-phase-boundary.sh exists', () => {
    const hookPath = path.join(HOOKS_DIR, 'brief-phase-boundary.sh');
    assert.ok(fs.existsSync(hookPath), 'brief-phase-boundary.sh should exist');
  });

  test('brief-session-state.sh is executable', { skip: isWindows ? 'Windows has no POSIX file permissions' : false }, () => {
    const hookPath = path.join(HOOKS_DIR, 'brief-session-state.sh');
    const stat = fs.statSync(hookPath);
    assert.ok((stat.mode & 0o111) !== 0, 'brief-session-state.sh should be executable');
  });

  test('brief-validate-commit.sh is executable', { skip: isWindows ? 'Windows has no POSIX file permissions' : false }, () => {
    const hookPath = path.join(HOOKS_DIR, 'brief-validate-commit.sh');
    const stat = fs.statSync(hookPath);
    assert.ok((stat.mode & 0o111) !== 0, 'brief-validate-commit.sh should be executable');
  });

  test('brief-phase-boundary.sh is executable', { skip: isWindows ? 'Windows has no POSIX file permissions' : false }, () => {
    const hookPath = path.join(HOOKS_DIR, 'brief-phase-boundary.sh');
    const stat = fs.statSync(hookPath);
    assert.ok((stat.mode & 0o111) !== 0, 'brief-phase-boundary.sh should be executable');
  });
});
...
describe('installer hook registration', () => {
  ...
  test('install.js contains brief-validate-commit registration block', () => {
    assert.ok(
      installSource.includes('brief-validate-commit'),
      'install.js should contain brief-validate-commit hook registration'
    );
    assert.ok(
  ...
  test('install.js contains brief-session-state registration block', () => {
    assert.ok(
      installSource.includes('brief-session-state'),
      'install.js should contain brief-session-state hook registration'
    );
    assert.ok(
  ...
  test('install.js contains brief-phase-boundary registration block', () => {
    assert.ok(
      installSource.includes('brief-phase-boundary'),
      'install.js should contain brief-phase-boundary hook registration'
    );
    assert.ok(
  ...
  test('install.js adds all 3 new hooks to the uninstall cleanup list', () => {
    const briefHooksMatch = installSource.match(/const briefHooks\s*=\s*\[([^\]]+)\]/);
    assert.ok(briefHooksMatch, 'install.js should define briefHooks array for uninstall cleanup');

    const briefHooksContent = briefHooksMatch[1];
    assert.ok(
      briefHooksContent.includes('brief-session-state.sh'),
      'briefHooks should include brief-session-state.sh'
    );
    assert.ok(
      briefHooksContent.includes('brief-validate-commit.sh'),
      'briefHooks should include brief-validate-commit.sh'
    );
    assert.ok(
      briefHooksContent.includes('brief-phase-boundary.sh'),
      'briefHooks should include brief-phase-boundary.sh'
    );
  });
