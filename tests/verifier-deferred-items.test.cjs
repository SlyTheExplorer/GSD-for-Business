 ...
 *
 * Verifies that the brief-verifier agent filters gaps addressed in later
...
describe('verifier deferred-items filtering (#1624)', () => {
  ...
  describe('agents/brief-verifier.md', () => {
    ...
    test('contains Step 9b for filtering deferred items', () => {
      ...
      assert.ok(
        ...
        'brief-verifier.md should contain Step 9b or "Filter Deferred" section'
      );
    });
  ...
  describe('brief/workflows/verify-phase.md', () => {
    const workflowPath = path.join(ROOT, 'brief', 'workflows', 'verify-phase.md');
    let workflowContent;
  ...
  describe('brief/references/planner-gap-closure.md', () => {
    const closurePath = path.join(ROOT, 'brief', 'references', 'planner-gap-closure.md');
    let closureContent;
