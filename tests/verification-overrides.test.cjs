 ...
 * the YAML frontmatter override format, and is referenced by brief-verifier.md.
 */
...
describe('verification overrides reference (#1747)', () => {
  ...
  describe('brief/references/verification-overrides.md', () => {
    const refPath = path.join(ROOT, 'brief', 'references', 'verification-overrides.md');
    let content;
  ...
  describe('agents/brief-verifier.md references overrides', () => {
    const verifierPath = path.join(ROOT, 'agents', 'brief-verifier.md');
    let verifierContent;

    test('brief-verifier.md exists', () => {
      assert.ok(fs.existsSync(verifierPath), 'brief-verifier.md should exist');
    ...
    test('references verification-overrides.md in required_reading', () => {
      ...
      assert.ok(
        ...
        'brief-verifier.md should reference verification-overrides.md'
      );
    });
    ...
    test('verifier includes Step 3b for override check before FAIL', () => {
      ...
      assert.ok(
        ...
        'brief-verifier.md should include a Step 3b override check'
      );
    });
    ...
    test('verifier frontmatter template includes overrides_applied', () => {
      ...
      assert.ok(
        ...
        'brief-verifier.md frontmatter template should include overrides_applied counter'
      );
    });
  });
});
