...
describe('agent-skills command', () => {
  ...
  test('returns empty when no config exists', () => {
    ...
    const result = runGsdTools(['agent-skills', 'brief-executor'], tmpDir, { HOME: tmpDir, USERPROFILE: tmpDir });
  ...
  test('returns empty for unconfigured agent type', () => {
    writeConfig(tmpDir, {
      agent_skills: {
        'brief-executor': ['skills/test-skill'],
      },
    });
    const result = runGsdTools(['agent-skills', 'brief-planner'], tmpDir, { HOME: tmpDir, USERPROFILE: tmpDir });
    assert.strictEqual(result.output, '');
  });
  ...
  test('returns formatted block for configured agent with single string path', () => {
    ...
    writeConfig(tmpDir, {
      agent_skills: {
        'brief-executor': 'skills/my-skill',
      },
    });
  ...
  test('handles multiple skill paths', () => {
    ...
    writeConfig(tmpDir, {
      agent_skills: {
        'brief-executor': ['skills/skill-a', 'skills/skill-b'],
      },
    });
  ...
  test('warns for nonexistent skill path but does not error', () => {
    writeConfig(tmpDir, {
      agent_skills: {
        'brief-executor': ['skills/nonexistent'],
      },
    });
...
describe('config-set agent_skills', () => {
  ...
  test('can set agent_skills via dot notation', () => {
    const result = runGsdTools(
      ['config-set', 'agent_skills.brief-executor', '["skills/my-skill"]'],
      tmpDir,
    ...
    assert.deepStrictEqual(
      config.agent_skills['brief-executor'],
      ['skills/my-skill'],
...
describe('agent-skills global: prefix', () => {
  ...
  test('global:valid-skill resolves to $HOME/.claude/skills/valid-skill/SKILL.md', () => {
    ...
    writeConfig(tmpDir, {
      agent_skills: { 'brief-executor': ['global:valid-skill'] },
    });

    const result = runGsdTools(['agent-skills', 'brief-executor'], tmpDir, { HOME: fakeHome, USERPROFILE: fakeHome });
  ...
  test('global:invalid!name is rejected by regex and skipped', () => {
    writeConfig(tmpDir, {
      agent_skills: { 'brief-executor': ['global:invalid!name'] },
    });
  ...
  test('global:missing-skill is skipped when directory is absent', () => {
    ...
    writeConfig(tmpDir, {
      agent_skills: { 'brief-executor': ['global:missing-skill'] },
    });
  ...
  test('mix of global: and project-relative paths both resolve correctly', () => {
    ...
    writeConfig(tmpDir, {
      agent_skills: { 'brief-executor': ['global:shadcn', 'skills/local-skill'] },
    });
  ...
  test('global: with empty name produces clear warning and skips', () => {
    writeConfig(tmpDir, {
      agent_skills: { 'brief-executor': ['global:'] },
    });
