...
import { GSDTools, GSDToolsError } from './brief-tools.js';
import {
  PhaseStepType,
  GSDEventType,
  PhaseType,
...
describe('Phase lifecycle types', () => {
  ...
  describe('PhaseOpInfo interface', () => {
    ...
    it('matches the documented init phase-op JSON shape', () => {
      // Simulate parsing JSON from brief-tools.cjs
...
describe('GSDTools typed methods', () => {
  ...
  beforeEach(async () => {
    tmpDir = join(tmpdir(), `brief-tools-phase-test-${Date.now()}-${Math.random().toString(36).slice(2)}`);
  ...
  describe('initPhaseOp()', () => {
    it('returns typed PhaseOpInfo from brief-tools output', async () => {
  ...
  describe('configGet()', () => {
    it('returns string value from brief-tools config', async () => {
      const scriptPath = await createScript(
        'config-get.cjs',
        `
        const args = process.argv.slice(2);
