...
**Current state (v1 launch):** 12 user-facing commands, 0 skills. Cap met for both. Audit: see `.planning/SURFACE-AUDIT.md`.

The 12 locked user-facing commands are: `/brief-define`, `/brief-discover`, `/brief-design`, `/brief-add-workstream`, `/brief-deliver`, `/brief-export`, `/brief-status`, `/brief-help`, `/brief-init`, `/brief-progress`, `/brief-undo`, `/brief-pause-work`. The 56 inherited GSD commands deleted in Phase 9 HRD-02 are preserved on the `backup/original-gsd` branch for recovery (`git checkout backup/original-gsd -- commands/brief/{name}.md`). The `new-project.md` file was RENAMED to `init.md` per the locked lineup; its content is preserved.
