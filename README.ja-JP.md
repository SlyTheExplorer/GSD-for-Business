...
だからBRIEFを作りました。複雑さはシステムの中にあり、ワークフローの中にはありません。裏側では、コンテキストエンジニアリング、XMLプロンプトフォーマッティング、サブエージェントのオーケストレーション、状態管理が動いています。あなたが目にするのは、ただ動くいくつかのコマンドだけです。

このシステムは、Claudeが仕事をし、*かつ*検証するために必要なすべてを提供します。私はこのワークフローを信頼しています。ちゃんといい仕事をしてくれます。

これがBRIEFです。エンタープライズごっこは一切なし。Claude Codeを使って一貫してクールなものを作るための、非常に効果的なシステムです。
...
BRIEFはそれを解決します。Claude Codeを信頼性の高いものにするコンテキストエンジニアリングレイヤーです。アイデアを説明し、システムに必要なすべてを抽出させ、Claude Codeに仕事をさせましょう。
...
- Codex: `$gsd-help`
- Cline: BRIEFは`.clinerules`経由でインストール — `.clinerules`の存在を確認

> [!NOTE]
> Claude Code 2.1.88+とCodexはスキル（`skills/brief-*/SKILL.md`）としてインストールされます。Clineは`.clinerules`を使用します。インストーラーがすべての形式を自動的に処理します。
...
BRIEFは急速に進化しています。定期的にアップデートしてください：
...
```bash
# Claude Code
npx brief-cc --claude --global   # ~/.claude/ にインストール
npx brief-cc --claude --local    # ./.claude/ にインストール

# OpenCode
npx brief-cc --opencode --global # ~/.config/opencode/ にインストール

# Gemini CLI
npx brief-cc --gemini --global   # ~/.gemini/ にインストール

# Kilo
npx brief-cc --kilo --global     # ~/.config/kilo/ にインストール
npx brief-cc --kilo --local      # ./.kilo/ にインストール

# Codex
npx brief-cc --codex --global    # ~/.codex/ にインストール
npx brief-cc --codex --local     # ./.codex/ にインストール

# Copilot
npx brief-cc --copilot --global  # ~/.github/ にインストール
npx brief-cc --copilot --local   # ./.github/ にインストール

# Cursor CLI
npx brief-cc --cursor --global      # ~/.cursor/ にインストール
npx brief-cc --cursor --local       # ./.cursor/ にインストール

# Antigravity
npx brief-cc --antigravity --global # ~/.gemini/antigravity/ にインストール
npx brief-cc --antigravity --local  # ./.agent/ にインストール

# Augment
npx brief-cc --augment --global     # ~/.augment/ にインストール
npx brief-cc --augment --local      # ./.augment/ にインストール

# Trae
npx brief-cc --trae --global        # ~/.trae/ にインストール
npx brief-cc --trae --local         # ./.trae/ にインストール

# Cline
npx brief-cc --cline --global       # ~/.cline/ にインストール
npx brief-cc --cline --local        # ./.clinerules にインストール

# 全ランタイム
npx brief-cc --all --global      # すべてのディレクトリにインストール
```
...
BRIEFは摩擦のない自動化のために設計されています。Claude Codeを以下のように実行してください：
...
> [!TIP]
> これがBRIEFの意図された使い方です — `date` や `git commit` を50回も承認するために止まっていては目的が台無しです。
...
> **既存のコードがある場合は？** まず `/brief-map-codebase` を実行してください。並列エージェントが起動し、スタック、アーキテクチャ、規約、懸念点を分析します。その後 `/brief-new-project` がコードベースを把握した状態で動作し、質問は追加する内容に焦点を当て、計画時にはパターンが自動的に読み込まれます。
...
> **前提モード：** 質問よりもコードベース分析を優先したい場合は、`/brief-settings` で `workflow.discuss_mode` を `assumptions` に設定してください。システムがコードを読み、何をなぜそうするかを提示し、間違っている部分だけ修正を求めます。詳しくは[ディスカスモード](docs/ja-JP/workflow-discuss-mode.md)をご覧ください。
...
すべてパスすれば次に進みます。何か壊れていれば、手動でデバッグする必要はありません — 作成された修正プランで `/brief-execute-phase` を再度実行するだけです。
...
/brief-verify-work 2
/brief-ship 2                  # 検証済みの作業からPRを作成
...
/brief-complete-milestone
/brief-new-milestone
```

またはBRIEFに次のステップを自動判定させます：

```
/brief-next                    # 次のステップを自動検出して実行
```

**discuss → plan → execute → verify → ship** のループをマイルストーン完了まで繰り返します。

ディスカッション中のインプットを速くしたい場合は、`/brief-discuss-phase <n> --batch` で1つずつではなく小さなグループにまとめた質問に一括で回答できます。`--chain` を使うと、ディスカッションからプラン+実行まで途中で止まらずに自動チェインできます。

各フェーズであなたのインプット（discuss）、適切なリサーチ（plan）、クリーンな実行（execute）、人間による検証（verify）が行われます。コンテキストは常にフレッシュ。品質は常に高い。

すべてのフェーズが完了したら、`/brief-complete-milestone` でマイルストーンをアーカイブしリリースをタグ付けします。

次に `/brief-new-milestone` で次のバージョンを開始します — `new-project` と同じフローですが既存のコードベース向けです。次に構築したいものを説明し、システムがドメインを調査し、要件をスコーピングし、新しいロードマップを作成します。各マイルストーンはクリーンなサイクルです：定義 → 構築 → シップ。
...
クイックモードはBRIEFの保証（アトミックコミット、状態トラッキング）をより速いパスで提供します：
...
**`--full` フラグ：** 全フェーズを有効化 — ディスカッション + リサーチ + プランチェック + 検証。クイックタスク形式のフルBRIEFパイプライン。
...
| コマンド | 説明 |
|---------|--------------|
| `/brief-new-project [--auto]` | フル初期化：質問 → リサーチ → 要件定義 → ロードマップ |
| `/brief-discuss-phase [N] [--auto] [--analyze] [--chain]` | 計画前に実装の決定事項をキャプチャ（`--analyze` でトレードオフ分析を追加、`--chain` でプラン+実行へ自動チェイン） |
| `/brief-plan-phase [N] [--auto] [--reviews]` | フェーズのリサーチ + プラン + 検証（`--reviews` でコードベースレビューの発見事項を読み込み） |
| `/brief-execute-phase <N>` | 全プランを並列ウェーブで実行し、完了時に検証 |
| `/brief-verify-work [N]` | 手動ユーザー受入テスト ¹ |
| `/brief-ship [N] [--draft]` | 検証済みのフェーズ作業から自動生成された本文付きのPRを作成 |
| `/brief-next` | 次の論理的なワークフローステップに自動的に進む |
| `/brief-fast <text>` | インラインの軽微タスク — 計画を完全にスキップし即座に実行 |
| `/brief-audit-milestone` | マイルストーンが完了の定義を達成したか検証 |
| `/brief-complete-milestone` | マイルストーンをアーカイブし、リリースをタグ付け |
| `/brief-new-milestone [name]` | 次のバージョンを開始：質問 → リサーチ → 要件定義 → ロードマップ |
| `/brief-forensics [desc]` | 失敗したワークフロー実行の事後分析（停止ループ、欠落成果物、git異常の診断） |
| `/brief-milestone-summary [version]` | チームオンボーディングとレビュー向けの包括的なプロジェクトサマリーを生成 |

### ワークストリーム

| コマンド | 説明 |
|---------|--------------|
| `/brief-workstreams list` | 全ワークストリームとそのステータスを表示 |
| `/brief-workstreams create <name>` | 並列マイルストーン作業用の名前空間付きワークストリームを作成 |
| `/brief-workstreams switch <name>` | アクティブなワークストリームを切り替え |
| `/brief-workstreams complete <name>` | ワークストリームを完了しマージ |

### マルチプロジェクトワークスペース

| コマンド | 説明 |
|---------|--------------|
| `/brief-new-workspace` | リポジトリのコピー（worktreeまたはクローン）で隔離されたワークスペースを作成 |
| `/brief-list-workspaces` | すべてのBRIEFワークスペースとそのステータスを表示 |
| `/brief-remove-workspace` | ワークスペースを削除しworktreeをクリーンアップ |

### UIデザイン

| コマンド | 説明 |
|---------|--------------|
| `/brief-ui-phase [N]` | フロントエンドフェーズ用のUIデザイン契約（UI-SPEC.md）を生成 |
| `/brief-ui-review [N]` | 実装済みフロントエンドコードの6つの柱によるビジュアル監査（遡及的） |

### ナビゲーション

| コマンド | 説明 |
|---------|--------------|
| `/brief-progress` | 今どこにいる？次は何？ |
| `/brief-next` | 状態を自動検出し次のステップを実行 |
| `/brief-help` | 全コマンドと使い方ガイドを表示 |
| `/brief-update` | チェンジログプレビュー付きでBRIEFをアップデート |
| `/brief-join-discord` | BRIEF Discordコミュニティに参加 |
| `/brief-manager` | 複数フェーズ管理用のインタラクティブコマンドセンター |

### ブラウンフィールド

| コマンド | 説明 |
|---------|--------------|
| `/brief-map-codebase [area]` | new-project前に既存のコードベースを分析 |

### フェーズ管理

| コマンド | 説明 |
|---------|--------------|
| `/brief-add-phase` | ロードマップにフェーズを追加 |
| `/brief-insert-phase [N]` | フェーズ間に緊急作業を挿入 |
| `/brief-remove-phase [N]` | 将来のフェーズを削除し番号を振り直し |
| `/brief-list-phase-assumptions [N]` | 計画前にClaudeの意図するアプローチを確認 |
| `/brief-plan-milestone-gaps` | 監査で見つかったギャップを埋めるフェーズを作成 |

### セッション

| コマンド | 説明 |
|---------|--------------|
| `/brief-pause-work` | フェーズ途中で停止する際の引き継ぎを作成（HANDOFF.jsonを書き込み） |
| `/brief-resume-work` | 前回のセッションから復元 |
| `/brief-session-report` | 実行した作業と結果のセッションサマリーを生成 |

### ワークストリーム

| コマンド | 説明 |
|---------|--------------|
| `/brief-workstreams` | 並列ワークストリームを管理（list、create、switch、status、progress、complete） |

### コード品質

| コマンド | 説明 |
|---------|--------------|
| `/brief-review` | 現在のフェーズまたはブランチのクロスAIピアレビュー |
| `/brief-pr-branch` | `.planning/` コミットをフィルタリングしたクリーンなPRブランチを作成 |
| `/brief-audit-uat` | 検証負債を監査 — UATが未実施のフェーズを検出 |

### バックログ & スレッド

| コマンド | 説明 |
|---------|--------------|
| `/brief-plant-seed <idea>` | トリガー条件付きの将来志向のアイデアをキャプチャ — 適切なマイルストーンで浮上 |
| `/brief-add-backlog <desc>` | バックログのパーキングロットにアイデアを追加（999.xナンバリング、アクティブシーケンス外） |
| `/brief-review-backlog` | バックログ項目をレビューし、アクティブマイルストーンに昇格またはstaleエントリを削除 |
| `/brief-thread [name]` | 永続コンテキストスレッド — 複数セッションにまたがる作業用の軽量クロスセッション知識 |

### ユーティリティ

| コマンド | 説明 |
|---------|--------------|
| `/brief-settings` | モデルプロファイルとワークフローエージェントを設定 |
| `/brief-set-profile <profile>` | モデルプロファイルを切り替え（quality/balanced/budget/inherit） |
| `/brief-add-todo [desc]` | 後で取り組むアイデアをキャプチャ |
| `/brief-check-todos` | 保留中のtodoを一覧表示 |
| `/brief-debug [desc]` | 永続状態を持つ体系的デバッグ |
| `/brief-do <text>` | フリーフォームテキストを適切なBRIEFコマンドに自動ルーティング |
| `/brief-note <text>` | ゼロフリクションのアイデアキャプチャ — ノートの追加、一覧、todoへの昇格 |
| `/brief-quick [--full] [--discuss] [--research]` | BRIEFの保証付きでアドホックタスクを実行（`--full` で全フェーズを有効化、`--discuss` で事前にコンテキストを収集、`--research` で計画前にアプローチを調査） |
| `/brief-health [--repair]` | `.planning/` ディレクトリの整合性を検証、`--repair` で自動修復 |
| `/brief-stats` | プロジェクト統計を表示 — フェーズ、プラン、要件、gitメトリクス |
| `/brief-profile-user [--questionnaire] [--refresh]` | セッション分析から開発者行動プロファイルを生成し、パーソナライズされた応答を提供 |
...
BRIEFはプロジェクト設定を `.planning/config.json` に保存します。`/brief-new-project` 実行時に設定するか、後から `/brief-settings` で更新できます。完全な設定スキーマ、ワークフロートグル、gitブランチオプション、エージェントごとのモデル内訳については、[ユーザーガイド](docs/ja-JP/USER-GUIDE.md#configuration-reference)をご覧ください。
...
これらのトグルには `/brief-settings` を使用するか、呼び出し時にオーバーライドできます：
...
**戦略：**
- **`none`** — 現在のブランチにコミット（デフォルトのBRIEF動作）
- **`phase`** — フェーズごとにブランチを作成し、フェーズ完了時にマージ
- **`milestone`** — マイルストーン全体で1つのブランチを作成し、完了時にマージ

マイルストーン完了時、BRIEFはスカッシュマージ（推奨）または履歴付きマージを提案します。
...
BRIEFはv1.27以降、多層防御セキュリティを備えています：
...
> [!NOTE]
> BRIEFはLLMシステムプロンプトとなるマークダウンファイルを生成するため、計画成果物に流入するユーザー制御テキストは潜在的な間接プロンプトインジェクションベクトルとなります。これらの保護は、そのようなベクトルを複数のレイヤーで捕捉するように設計されています。

### 機密ファイルの保護

BRIEFのコードベースマッピングおよび分析コマンドは、プロジェクトを理解するためにファイルを読み取ります。**シークレットを含むファイルを保護する**には、Claude Codeの拒否リストに追加してください：
...
> [!IMPORTANT]
> BRIEFにはシークレットのコミットに対する組み込み保護がありますが、多層防御がベストプラクティスです。防御の第一線として、機密ファイルへの読み取りアクセスを拒否してください。
...
**コマンドが期待通りに動作しない？**
- `/brief-help` を実行してインストールを確認してください
- `npx brief-cc` を再実行して再インストールしてください
...
これにより、他の設定を保持しながら、すべてのBRIEFコマンド、エージェント、フック、設定が削除されます。

---

## コミュニティポート

OpenCode、Gemini CLI、Kilo、Codexは `npx brief-cc` でネイティブサポートされています。

以下のコミュニティポートがマルチランタイムサポートの先駆けとなりました：

| プロジェクト | プラットフォーム | 説明 |
|---------|----------|-------------|
| [gsd-opencode](https://github.com/rokicool/brief-opencode) | OpenCode | オリジナルのOpenCode対応版 |
...
**Claude Codeは強力です。BRIEFはそれを信頼性の高いものにします。**
